...
[ScriptType(name: "M2s绘图", territorys: [1228], guid: "d99c7e91-9b56-432d-a3a8-49a8586915b7e2a", version: "0.0.0.3",
    author: "Karlin")]
public class M2s绘图绘图
{
    private readonly object towerLock = new();
    private int TowerCount;

    private int beeChargeCount1;


    private int? firstTargetIcon;

    private bool longCircle;
    private int parse;
    private bool party2Circle;
    private int[] party2HeartStart = [-1, -1, -1, -1, -1, -1, -1, -1];
    private bool spreadFlow;

    [UserSetting("地面爱心轨迹长度")] public float heartLenth { get; set; } = 4;
    ...
    [ScriptMethod(name: "直播1记录", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:39972"],
        userControl: false)]
    ...
    [ScriptMethod(name: "直播2记录", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:39973"],
        userControl: false)]
    ...
    [ScriptMethod(name: "直播3记录", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:39974"],
        userControl: false)]
    ...
    //39626 月环 30
    //39625 直线 14,60
    [ScriptMethod(name: "分散分摊收集", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3725[23]|3968[89])$"], userControl: false)]
    ...
    [ScriptMethod(name: "直线斩", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(37254|39692)$"])]
    ...
    [ScriptMethod(name: "月环", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(37255|39693)$"])]
    ...
    [ScriptMethod(name: "分散分摊", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:regex:^(3725[45]|3969[23])$"])]
    public void 分散分摊(Event @event, ScriptAccessory accessory)
    {
        ...
        else
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                ...
                dp.Name = "分摊";
                dp.Scale = new Vector2(6);
                dp.Color = i == myIndex || i == myPartener
    ...
    public void 四四分摊(Event @event, ScriptAccessory accessory)
    {
        ...
        int[] TNGroup = [0, 1, 2, 3];
        var isMyGroup = TNGroup.Contains(accessory.Data.PartyList.IndexOf(tid)) ==
                        TNGroup.Contains(accessory.Data.PartyList.IndexOf(accessory.Data.Me));
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = "地面黄圈";
        dp.Scale = new Vector2(6);
    ...
    [ScriptMethod(name: "地面黄圈", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:regex:^(37229|39690)$"])]
    ...
    [ScriptMethod(name: "三连组合技", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3729[23])$"])]
    ...
    [ScriptMethod(name: "小蜜蜂冲锋", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3962[78])$"])]
    ...
    public void 地面爱心(Event @event, ScriptAccessory accessory)
    {
        ...
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"爱心{sid}";
        dp.Scale = new Vector2(2f, heartLenth);
    ...
    [ScriptMethod(name: "直播1地面爱心删除", eventType: EventTypeEnum.RemoveCombatant, eventCondition: ["DataId:16943"],
        userControl: false)]
    ...
    [ScriptMethod(name: "直播1地面爱心删除2", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:37285"],
        userControl: false)]
    ...
    [ScriptMethod(name: "直播2爱心Buff收集", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(392[23456])$"], userControl: false)]
    ...
    public void 直播2放圈收集(Event @event, ScriptAccessory accessory)
    {
        ...
        if (!ParseObjectId(@event["TargetId"], out var tid)) return;
        if (tid == accessory.Data.Me) party2Circle = true;
    }
    ...
    public void 直播2踩塔(Event @event, ScriptAccessory accessory)
    {
        ...
        lock (towerLock)
        {
            ...
            var dir4 = PositionRoundTo4Dir(pos, new Vector3(100, 0, 100));
            var ismy = false;
    ...
    [ScriptMethod(name: "直播3大圈长短收集", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:3934"],
        userControl: false)]
    ...
    public void 直播3踩塔(Event @event, ScriptAccessory accessory)
    {
        ...
        lock (towerLock)
        {
            ...
            var pos = JsonConvert.DeserializeObject<Vector3>(@event["SourcePosition"]);
            var index = accessory.Data.PartyList.IndexOf(accessory.Data.Me);
            var dir4 = PositionFloorTo4Dir(pos, new Vector3(100, 0, 100));
            var ismy = false;
