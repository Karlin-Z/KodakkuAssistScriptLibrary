...
[ScriptType(name: "极艳翼蛇鸟绘图", territorys: [1196], guid: "d9c97e91-9b59-432d-a8a2-42a8586985b7e2a", version: "0.0.0.3",
    author: "Karlin")]
public class ValigarmandaExDraw
{
    private readonly object icelock = new();
    private int? firstTargetIcon;

    private bool iceCross;
    ...
    [ScriptMethod(name: "火阶段", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:38340"],
        userControl: false)]
    ...
    [ScriptMethod(name: "风阶段", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:38339"],
        userControl: false)]
    ...
    [ScriptMethod(name: "冰阶段", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:36817"],
        userControl: false)]
    ...
    public void 体操钢铁(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Color = accessory.Data.DefaultDangerColor;
        if (ParseObjectId(@event["SourceId"], out var tid)) dp.Owner = tid;
        dp.DestoryAt = 7300;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    }
    ...
    [ScriptMethod(name: "体操分摊", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(36808|36812|36816)$"])]
    ...
    [ScriptMethod(name: "火山喷发左", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["Id:00200010", "Index:0000000F"])]
    public void 火山喷发左(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Position = new Vector3(85, 0, 100);
        dp.DestoryAt = 11000;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    }

    [ScriptMethod(name: "火山喷发右", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["Id:00200010", "Index:0000000E"])]
    public void 火山喷发右(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Scale = new Vector2(22);
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Position = new Vector3(115, 0, 100);
        dp.DestoryAt = 11000;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    }
    ...
    public void 冰花米字安全区(Event @event, ScriptAccessory accessory)
    {
        lock (icelock)
        {
            ...
            if ((pos - pos11).Length() < 1f || (pos - pos12).Length() < 1f || (pos - pos13).Length() < 1f ||
                (pos - pos14).Length() < 1f || (pos - pos15).Length() < 1f)
            {
                iceCross = true;
                Task.Delay(1000).ContinueWith(y => { iceCross = false; });
                var dp = accessory.Data.GetDefaultDrawProperties();
                dp.Name = "冰花米字安全区";
                dp.Scale = new Vector2(2.07f, 3.54f);
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.Position = new Vector3(108.23f, 0, 91.77f);
            ...
            if ((pos - pos21).Length() < 1f || (pos - pos22).Length() < 1f || (pos - pos23).Length() < 1f ||
                (pos - pos24).Length() < 1f || (pos - pos25).Length() < 1f)
            {
                iceCross = true;
                Task.Delay(1000).ContinueWith(y => { iceCross = false; });
                var dp = accessory.Data.GetDefaultDrawProperties();
                ...
                dp.Scale = new Vector2(2.07f, 3.54f);
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.Position = new Vector3(91.77f, 0, 91.77f);
                dp.Rotation = float.Pi / 4;
                dp.DestoryAt = 5000;
    ...
    public void 分摊打柱子(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Color = accessory.Data.DefaultSafeColor;
        if (ParseObjectId(@event["SourceId"], out var cid)) dp.Owner = cid;
        if (ParseObjectId(@event["TargetId"], out var tid)) dp.TargetObject = tid;
        dp.DestoryAt = 99000;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Rect, dp);
    }

    [ScriptMethod(name: "移除分摊范围", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:38245"],
        userControl: false)]
