...
[ScriptType(name: "M5s绘图", territorys: [1257], guid: "ecf3365e-8d21-5daa-0329-8aa33ee30778", version: "0.0.0.3",
...
public class M5sDraw
{
    ...
    private readonly List<DateTime> wavelengthBuffEndTime =
    ...
    [UserSetting("左右刀分摊延迟显示时间")] public uint StackDelay { get; set; } = 5000;

    [UserSetting("分身左右刀间隔")] public int filpDuring { get; set; } = 2450;
    ...
    [ScriptMethod(name: "BossId记录", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:41767", "TargetIndex:1"], userControl: false)]
    ...
    [ScriptMethod(name: "分摊方式记录", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(42880|42881)$"], userControl: false)]
    public void 分摊方式记录(Event @event, ScriptAccessory accessory)
    {
        isHealerStack = @event.ActionId == 42881;
    }

    [ScriptMethod(name: "左右刀", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4279[234567]|4280[0123459]|4281[01234]|4220[345678])$"])]
    ...
    [ScriptMethod(name: "左右刀对穿提示", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4279[234567]|4280[0123459]|4281[01234]|4220[345678])$"])]
    ...
    [ScriptMethod(name: "左右刀分组分摊", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4279[234567]|4280[0123459]|4281[01234]|4220[345678])$"])]
    ...
    [ScriptMethod(name: "舞会阶段重置", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:42838"],
        userControl: false)]
    public void 舞会阶段重置(Event @event, ScriptAccessory accessory)
    {
        ...
        fireCount = 0;
        parse++;
        SpotlightResetEvent = new ManualResetEvent(false);
        BurnBuffResetEvent = new ManualResetEvent(false);
        spotLightRound = 0;
    }

    [ScriptMethod(name: "内圈聚光灯顺逆记录", eventType: EventTypeEnum.PlayActionTimeline,
        eventCondition: ["Id:4572", "SourceDataId:18363"], userControl: false)]
    ...
    [ScriptMethod(name: "地火情况记录", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["Index:3", "Flag:regex:^(2|32)$"], userControl: false)]
    ...
    [ScriptMethod(name: "地火绘图", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["Index:3", "Flag:regex:^(8|128)$"])]
    ...
    public void 第一次燃起来Buff处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(dur - 8000).ContinueWith(t =>
        {
            ...
            if (myindex == 2 || myindex == 6)
                dealpos = fireSafePoint == 2 ? new Vector3(87.5f, 0, 87.5f) : dealpos = new Vector3(87.5f, 0, 112.5f);
            if (myindex == 3 || myindex == 7)
                dealpos = fireSafePoint == 2 ? new Vector3(112.5f, 0, 112.5f) : dealpos = new Vector3(112.5f, 0, 87.5f);
            if (myindex == 0 || myindex == 4)
            {
                if (first)
                {
                    if (fireSafePoint == 1)
                        dealpos = spotLightClockwise ? new Vector3(97.5f, 0, 92.5f) : new Vector3(92.5f, 0, 97.5f);
                    if (fireSafePoint == 2)
                        dealpos = spotLightClockwise ? new Vector3(92.5f, 0, 102.5f) : new Vector3(97.5f, 0, 107.5f);
                }
                else
                {
                    if (fireSafePoint == 1)
                        dealpos = spotLightClockwise ? new Vector3(92.5f, 0, 97.5f) : new Vector3(97.5f, 0, 92.5f);
                    if (fireSafePoint == 2)
                        dealpos = spotLightClockwise ? new Vector3(97.5f, 0, 107.5f) : new Vector3(92.5f, 0, 102.5f);
                }
            }
            ...
            if (myindex == 1 || myindex == 5)
            {
                ...
                {
                    if (fireSafePoint == 1)
                        dealpos = spotLightClockwise ? new Vector3(102.5f, 0, 107.5f) : new Vector3(107.5f, 0, 102.5f);
                    if (fireSafePoint == 2)
                        dealpos = spotLightClockwise ? new Vector3(107.5f, 0, 97.5f) : new Vector3(102.5f, 0, 92.5f);
                }
                else
                {
                    if (fireSafePoint == 1)
                        dealpos = spotLightClockwise ? new Vector3(107.5f, 0, 102.5f) : new Vector3(102.5f, 0, 107.5f);
                    if (fireSafePoint == 2)
                        dealpos = spotLightClockwise ? new Vector3(102.5f, 0, 92.5f) : new Vector3(107.5f, 0, 97.5f);
                }
            }
    ...
    [ScriptMethod(name: "二次燃起来聚光灯记录", eventType: EventTypeEnum.PlayActionTimeline,
        eventCondition: ["Id:4572", "SourceDataId:18363"], userControl: false)]
    ...
    public void 第二次燃起来Buff处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(dur - 9500).ContinueWith(t =>
        {
            ...
            {
                if (spotLightN)
                    dealpos = first ? new Vector3(85, 0, 85) : new Vector3(100, 0, 85);
                else
                    dealpos = first ? new Vector3(100, 0, 85) : new Vector3(85, 0, 85);
            }

            if (myindex == 1 || myindex == 5)
            {
                if (spotLightN)
                    dealpos = first ? new Vector3(115, 0, 115) : new Vector3(100, 0, 115);
                else
                    dealpos = first ? new Vector3(100, 0, 115) : new Vector3(115, 0, 115);
            }

            if (myindex == 2 || myindex == 4)
            {
                if (spotLightN)
                    dealpos = first ? new Vector3(85, 0, 115) : new Vector3(85, 0, 100);
                else
                    dealpos = first ? new Vector3(85, 0, 100) : new Vector3(85, 0, 115);
            }

            if (myindex == 3 || myindex == 7)
            {
                if (spotLightN)
                    dealpos = first ? new Vector3(115, 0, 85) : new Vector3(115, 0, 100);
                else
                    dealpos = first ? new Vector3(115, 0, 100) : new Vector3(115, 0, 85);
            }
    ...
    [ScriptMethod(name: "第二次燃起来Buff记录", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:4461"],
        userControl: false)]
    ...
    [ScriptMethod(name: "二次燃起来引导位置", eventType: EventTypeEnum.PlayActionTimeline,
        eventCondition: ["Id:4561", "SourceDataId:18362"], suppress: 1000)]
    public void 二次燃起来引导位置(Event @event, ScriptAccessory accessory)
    {
        ...
        {
            if (nowSpotlightN)
                dealpos = danceN ? new Vector3(98, 0, 93) : new Vector3(93, 0, 93);
            else
                dealpos = danceN ? new Vector3(100, 0, 91) : new Vector3(95, 0, 93);
        }

        if (myindex == 1 || myindex == 5)
        {
            if (nowSpotlightN)
                dealpos = danceN ? new Vector3(102, 0, 107) : new Vector3(107, 0, 107);
            else
                dealpos = danceN ? new Vector3(100, 0, 109) : new Vector3(105, 0, 107);
        }

        if (myindex == 2 || myindex == 4)
        {
            if (nowSpotlightN)
                dealpos = danceN ? new Vector3(93, 0, 102) : new Vector3(93, 0, 107);
            else
                dealpos = danceN ? new Vector3(91, 0, 100) : new Vector3(93, 0, 105);
        }

        if (myindex == 3 || myindex == 7)
        {
            if (nowSpotlightN)
                dealpos = danceN ? new Vector3(107, 0, 98) : new Vector3(107, 0, 93);
            else
                dealpos = danceN ? new Vector3(109, 0, 100) : new Vector3(105, 0, 93);
        }
    ...
    [ScriptMethod(name: "钢铁月环", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(42876|42878)$"])]
    ...
    [ScriptMethod(name: "分身阶段重置", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:39474"],
        userControl: false)]
    ...
    [ScriptMethod(name: "分身左右刀记录", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4276[2345])$"], userControl: false)]
    ...
    [ScriptMethod(name: "Wavelenth记录", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(4463|4462)$"], userControl: false)]
    ...
    [ScriptMethod(name: "Wavelenth搭档", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(4463|4462)$"])]
    ...
    [ScriptMethod(name: "Wavelenth分摊提醒", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(4463|4462)$"])]
    ...
    [ScriptMethod(name: "舞浪全开拉怪TTS", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(42836)$"])]
    ...
    public void 分身左右刀(Event @event, ScriptAccessory accessory)
    {
        if (parse != 2) return;
        var dur1 = 5800;
        for (var i = 1; i < flipDir.Count; i++)
        {
            if (flipDir[i] != flipDir[i - 1]) break;
            dur1 += filpDuring;
        }
        ...
        for (var i = 1; i < flipDir.Count; i++)
        {
            ...
            var dur = filpDuring;
            for (var j = i + 1; j < flipDir.Count; j++)
            {
                if (flipDir[j] != flipDir[j - 1]) break;
                dur += filpDuring;
            }
    ...
    [ScriptMethod(name: "分身前后左右刀范围预告", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(41872)$"])]
    public void 分身前后左右刀范围预告(Event @event, ScriptAccessory accessory)
    {
        if (parse != 6) return;

        var danger1Area = FlipDangerArea.None;
        var danger2Area = FlipDangerArea.None;
        var danger3Area = FlipDangerArea.None;
        var danger4Area = FlipDangerArea.None;
    ...
    [ScriptMethod(name: "分身前后左右刀对穿方式", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(41872)$"])]
    public void 分身前后左右刀对穿方式(Event @event, ScriptAccessory accessory)
    {
        ...
        if (!danger1Area.HasFlag(FlipDangerArea.Point1)) dealpos1 = new Vector3(101, 0, 99);
        if (!danger1Area.HasFlag(FlipDangerArea.Point2)) dealpos1 = new Vector3(101, 0, 101);
        if (!danger1Area.HasFlag(FlipDangerArea.Point3)) dealpos1 = new Vector3(99, 0, 101);
        if (!danger1Area.HasFlag(FlipDangerArea.Point4)) dealpos1 = new Vector3(99, 0, 99);
        Vector3 dealpos2 = default;
        if (!danger2Area.HasFlag(FlipDangerArea.Point1)) dealpos2 = new Vector3(101, 0, 99);
        if (!danger2Area.HasFlag(FlipDangerArea.Point2)) dealpos2 = new Vector3(101, 0, 101);
        if (!danger2Area.HasFlag(FlipDangerArea.Point3)) dealpos2 = new Vector3(99, 0, 101);
        if (!danger2Area.HasFlag(FlipDangerArea.Point4)) dealpos2 = new Vector3(99, 0, 99);
        Vector3 dealpos3 = default;
        if (!danger3Area.HasFlag(FlipDangerArea.Point1)) dealpos3 = new Vector3(101, 0, 99);
        if (!danger3Area.HasFlag(FlipDangerArea.Point2)) dealpos3 = new Vector3(101, 0, 101);
        if (!danger3Area.HasFlag(FlipDangerArea.Point3)) dealpos3 = new Vector3(99, 0, 101);
        if (!danger3Area.HasFlag(FlipDangerArea.Point4)) dealpos3 = new Vector3(99, 0, 99);
        Vector3 dealpos4 = default;
        if (!danger4Area.HasFlag(FlipDangerArea.Point1)) dealpos4 = new Vector3(101, 0, 99);
        if (!danger4Area.HasFlag(FlipDangerArea.Point2)) dealpos4 = new Vector3(101, 0, 101);
        if (!danger4Area.HasFlag(FlipDangerArea.Point3)) dealpos4 = new Vector3(99, 0, 101);
        if (!danger4Area.HasFlag(FlipDangerArea.Point4)) dealpos4 = new Vector3(99, 0, 99);
    ...
    [ScriptMethod(name: "分身左右刀对穿提示", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(42858)$"])]
    ...
    [ScriptMethod(name: "箭头分身左右刀", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(42869|42870|42788|42789)$"])]
    ...
    [ScriptMethod(name: "俄罗斯方块重置", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:42836"],
        userControl: false)]
    ...
    [ScriptMethod(name: "俄罗斯方块 分摊分散", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(42844|42846)$"])]
    ...
    [ScriptMethod(name: "俄罗斯方块地火安全区绘图", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["Index:4", "Flag:regex:^(128|512)$"])]
    public void 俄罗斯方块地火安全区绘图(Event @event, ScriptAccessory accessory)
    {
        ...
        for (var i = 0; i < 15; i++)
        {
            ...
            dp.Position = new Vector3(left ? 87.5f : 112.5f, 0, 60.0f + i * 5);
            dp.Scale = new Vector2(5, 25);
            ...
            dp.Position = new Vector3(92.5f, 0, 45.0f + i * 5);
            ...
            dp.Position = new Vector3(left ? 102.5f : 97.5f, 0, 60.0f + i * 5);
            dp.Scale = new Vector2(5, 25);
            dp.Delay = i == 0 ? 0 : 3000 + (i - 1) * dur;
            dp.DestoryAt = i == 0 ? 3000 : dur;
            dp.Color = accessory.Data.DefaultSafeColor;
            ...
            dp.Position = new Vector3(107.5f, 0, 45.0f + i * 5);
            dp.Scale = new Vector2(5, 20);
            dp.Delay = i == 0 ? 0 : 3000 + (i - 1) * dur;
            dp.DestoryAt = i == 0 ? 3000 : dur;
            dp.Color = accessory.Data.DefaultSafeColor;
    ...
    [ScriptMethod(name: "太空步阶段重置", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:42847"],
        userControl: false)]
    ...
    [ScriptMethod(name: "太空步Aoe", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(42867|42868)$"])]
    public void 太空步Aoe(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Offset = new Vector3(@event["ActionId"] == "42867" ? 7.5f : -7.5f, 0, 0);
        dp.DestoryAt = 10500;
        dp.Color = accessory.Data.DefaultDangerColor;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Rect, dp);
    }

    [ScriptMethod(name: "太空步 分摊分散", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(42844|42846)$"])]
