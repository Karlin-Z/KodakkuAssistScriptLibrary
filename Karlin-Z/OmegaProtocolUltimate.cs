...
[ScriptType(name: "OmegaProtocolUltimate", territorys: [1122], guid: "625eb340-0811-4c37-b87c-c46fe5204940",
    author: "Karlin", version: "0.0.0.4", note: noteStr, updateInfo: updateInfoStr)]
public class OmegaProtocolUltimate
{
    ...
    private readonly List<int> HtdhParty = [2, 0, 1, 4, 5, 6, 7, 3];
    private readonly List<int> P1_点名Buff = [0, 0, 0, 0, 0, 0, 0, 0];
    private readonly List<int> P2_Sony = [0, 0, 0, 0, 0, 0, 0, 0];

    private uint P1_BossId;
    private DateTime P1_FanTime = DateTime.MinValue;
    private int P1_FireCount;
    private int P1_LineRound;
    private List<Vector3> P1_TowerPos = [];
    private DateTime P1_TowerTime = DateTime.MinValue;

    private bool P2_PTBuffIsFar;
    private List<int> P2_Stack = [0, 0, 0, 0, 0, 0, 0, 0];
    private Dictionary<uint, uint> P2_刀光剑舞连线 = [];

    private int P3_ArmCount;
    private List<int> P3_StartBuff = [0, 0, 0, 0, 0, 0, 0, 0];
    private bool P3_StartDone;
    private bool P3_StartPreDone;
    private List<int> P3_TVBuff = [0, 0, 0, 0, 0, 0, 0, 0];

    private List<int> P4Stack = [];
    private List<int> P51_Buff = [0, 0, 0, 0, 0, 0, 0, 0];

    private int P51_Eye;
    private List<int> P51_Fist = [0, 0, 0, 0];
    private bool P51_FistDone;
    private double parse;

    [UserSetting("P3_开场排队顺序")] public P3SortEnum P3_StackSort { get; set; }

    [UserSetting("P3_小电视打法")] public P3TVEnum P3_TV_Strategy { get; set; }
    ...
    [ScriptMethod(name: "P1_循环程序_分P", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:31491"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P1_循环程序_Buff记录", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(3004|3005|3006|3451)$"], userControl: false)]
    ...
    [ScriptMethod(name: "P1_循环程序_塔收集", eventType: EventTypeEnum.ObjectChanged,
        eventCondition: ["Operate:Add", "DataId:2013245"], userControl: false)]
    ...
    [ScriptMethod(name: "P1_循环程序_开始站位提醒", eventType: EventTypeEnum.StatusAdd,
    ...
    [ScriptMethod(name: "P1_循环程序_线塔处理位置", eventType: EventTypeEnum.ObjectChanged,
        eventCondition: ["Operate:Add", "DataId:2013245"])]
    public async void P1_循环程序_线塔处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        if (towerCount == myBuff * 2)
        {
            ...
            if (RoundPositionTo4Dir(P1_TowerPos[towerCount - 2], centre) <
                RoundPositionTo4Dir(P1_TowerPos[towerCount - 1], centre))
            {
        ...
        if (towerCount % 8 == (myBuff + 2) * 2 % 8)
        {
            ...
            var dealpos = RotatePoint(new Vector3(100, 0, 85), centre, float.Pi / 2 * my4Dir);
        ...
        if (!idle)
        {
            ...
            var drot = (myPos - P1_TowerPos[towerCount - 2]).Length() < (myPos - P1_TowerPos[towerCount - 1]).Length()
                ? RoundPositionTo4Dir(P1_TowerPos[towerCount - 2], centre)
                : RoundPositionTo4Dir(P1_TowerPos[towerCount - 1], centre);
            var dealpos = RotatePoint(new Vector3(100, 0, 86), centre, float.Pi / 2 * drot);
            var dp = accessory.Data.GetDefaultDrawProperties();
            dp.Name = "P1_循环程序_闲站位";
            dp.Scale = new Vector2(2);
            dp.Owner = accessory.Data.Me;
    ...
    [ScriptMethod(name: "P1_循环程序_接线标记", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:31496", "TargetIndex:1"])]
    ...
    [ScriptMethod(name: "P1_全能之主_分P", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:31499"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P1_全能之主_Buff记录", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(3004|3005|3006|3451)$"], userControl: false)]
    ...
    [ScriptMethod(name: "P1_全能之主_分摊范围", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(350[789]|3510)$"])]
    ...
    public void P1_全能之主_T引导顺劈位置(Event @event, ScriptAccessory accessory)
    {
        lock (this)
        {
            ...
            dp.Scale = new Vector2(2);
            dp.Owner = accessory.Data.Me;
            dp.TargetPosition = new Vector3(100, 0, 86);
            dp.ScaleMode |= ScaleMode.YByDistance;
            dp.Color = accessory.Data.DefaultSafeColor;
            dp.DestoryAt = 11000;
            accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
    ...
    [ScriptMethod(name: "P2_协同程序PT_分P", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:31550"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P2_协同程序PT_Buff记录", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(3427|3428)$"], userControl: false)]
    ...
    [ScriptMethod(name: "P2_协同程序PT_索尼记录", eventType: EventTypeEnum.TargetIcon,
        eventCondition: ["Id:regex:^(01A[0123])$"], userControl: false)]
    ...
    [ScriptMethod(name: "P2_协同程序PT_男女人AOE", eventType: EventTypeEnum.PlayActionTimeline,
        eventCondition: ["Id:7747", "SourceDataId:regex:^(15714|15715)$"])]
    public void P2_协同程序PT_男女人AOE(Event @event, ScriptAccessory accessory)
    {
        ...
        if (c == null) return;
        var transformationID = ((KodakkuAssist.Data.ICharacter)c).GetTransformationID();
        ...
        if (@event["SourceDataId"] == "15715")
        {
            ...
            if (transformationID == 4)
            {
                ...
                dp.Scale = new Vector2(60, 20);
                dp.Owner = sid;
                dp.Rotation = float.Pi / 2;
                dp.Offset = new Vector3(-5, 0, 0);
    ...
    [ScriptMethod(name: "P2_协同程序PT_眼睛激光", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["DirectorId:800375AC", "Id:00020001"])]
    public void P2_协同程序PT_眼睛激光(Event @event, ScriptAccessory accessory)
    {
        ...
        if (rot == -1) return;
        var pos = RotatePoint(new Vector3(100, 0, 80), new Vector3(100, 0, 100), float.Pi / 4 * rot);
        var dp = accessory.Data.GetDefaultDrawProperties();
    ...
    [ScriptMethod(name: "P2_协同程序PT_五钢铁", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:31521", "TargetIndex:1"])]
    ...
    [ScriptMethod(name: "P2_协同程序PT_眼睛激光索尼处理位置", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["DirectorId:800375AC", "Id:00020001"])]
    public async void P2_协同程序PT_眼睛激光索尼处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var middleLeft1Pos = RotatePoint(new Vector3(088.5f, 0, 085.5f), centre, float.Pi / 4 * dir);
        var middleRight1Pos = RotatePoint(new Vector3(111.5f, 0, 085.5f), centre, float.Pi / 4 * dir);
        var middleLeft2Pos = RotatePoint(new Vector3(088.5f, 0, 095.0f), centre, float.Pi / 4 * dir);
        var middleRight2Pos = RotatePoint(new Vector3(111.5f, 0, 095.0f), centre, float.Pi / 4 * dir);
        var middleLeft3Pos = RotatePoint(new Vector3(088.5f, 0, 105.0f), centre, float.Pi / 4 * dir);
        var middleRight3Pos = RotatePoint(new Vector3(111.5f, 0, 105.0f), centre, float.Pi / 4 * dir);
        var middleLeft4Pos = RotatePoint(new Vector3(088.5f, 0, 114.5f), centre, float.Pi / 4 * dir);
        var middleRight4Pos = RotatePoint(new Vector3(111.5f, 0, 114.5f), centre, float.Pi / 4 * dir);

        var farLeft1Pos = RotatePoint(new Vector3(091.5f, 0, 083.0f), centre, float.Pi / 4 * dir);
        var farRight1Pos = RotatePoint(new Vector3(108.5f, 0, 117.0f), centre, float.Pi / 4 * dir);
        var farLeft2Pos = RotatePoint(new Vector3(082.0f, 0, 093.0f), centre, float.Pi / 4 * dir);
        var farRight2Pos = RotatePoint(new Vector3(118.0f, 0, 107.0f), centre, float.Pi / 4 * dir);
        var farLeft3Pos = RotatePoint(new Vector3(082.0f, 0, 107.0f), centre, float.Pi / 4 * dir);
        var farRight3Pos = RotatePoint(new Vector3(118.0f, 0, 093.0f), centre, float.Pi / 4 * dir);
        var farLeft4Pos = RotatePoint(new Vector3(091.5f, 0, 117.0f), centre, float.Pi / 4 * dir);
        var farRight4Pos = RotatePoint(new Vector3(108.5f, 0, 083.0f), centre, float.Pi / 4 * dir);
    ...
    public void P2_协同程序PT_分摊处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var myindex = accessory.Data.PartyList.IndexOf(accessory.Data.Me);
        List<int> leftGroup = [];
        leftGroup.Add(HtdhParty.IndexOf(P2_Sony.IndexOf(1)) < HtdhParty.IndexOf(P2_Sony.LastIndexOf(1))
            ? P2_Sony.IndexOf(1)
            : P2_Sony.LastIndexOf(1));
        leftGroup.Add(HtdhParty.IndexOf(P2_Sony.IndexOf(2)) < HtdhParty.IndexOf(P2_Sony.LastIndexOf(2))
            ? P2_Sony.IndexOf(2)
            : P2_Sony.LastIndexOf(2));
        leftGroup.Add(HtdhParty.IndexOf(P2_Sony.IndexOf(3)) < HtdhParty.IndexOf(P2_Sony.LastIndexOf(3))
            ? P2_Sony.IndexOf(3)
            : P2_Sony.LastIndexOf(3));
        leftGroup.Add(HtdhParty.IndexOf(P2_Sony.IndexOf(4)) < HtdhParty.IndexOf(P2_Sony.LastIndexOf(4))
            ? P2_Sony.IndexOf(4)
            : P2_Sony.LastIndexOf(4));
        ...
        if (leftGroup.Contains(P2_Stack[0]) && leftGroup.Contains(P2_Stack[1]))
        {
            ...
            var lowStackPartnerIndex = P2_Sony.IndexOf(lowStackSony) == lowStackIndex
                ? P2_Sony.LastIndexOf(lowStackSony)
                : P2_Sony.IndexOf(lowStackSony);
        ...
        if (P2_PTBuffIsFar)
            dealpos = leftGroup.Contains(myindex) ? new Vector3(94, 0, 100) : new Vector3(106, 0, 100);
        else
            dealpos = leftGroup.Contains(myindex) ? new Vector3(97, 0, 100) : new Vector3(100, 0, 103);
        var c = accessory.Data.Objects.Where(o => o.DataId == 15713).FirstOrDefault();
        if (c == null) return;
        var dir8 = RoundPositionTo8Dir(c!.Position, new Vector3(100, 0, 100));
        //accessory.Log.Debug($"P2_协同程序PT {dir8} {leftGroup.Contains(myindex)}");
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = "P2_协同程序PT_分摊处理位置";
        dp.Scale = new Vector2(2);
        dp.Owner = accessory.Data.Me;
        dp.TargetPosition = RotatePoint(dealpos, new Vector3(100, 0, 100), float.Pi / 4 * dir8);
        dp.ScaleMode |= ScaleMode.YByDistance;
        dp.Color = accessory.Data.DefaultSafeColor;
        dp.DestoryAt = 7000;
        accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
    ...
    [ScriptMethod(name: "P2_协同程序LB_分P", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:31544"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P2_协同程序LB_刀光剑舞", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3154[01])$"])]
    ...
    [ScriptMethod(name: "P2_协同程序LB_盾连击S命中提示", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:31528"])]
    ...
    [ScriptMethod(name: "P2_协同程序LB_射手天剑引导位置", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:31544"])]
    public void P2_协同程序LB_射手天剑引导位置(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Scale = new Vector2(2);
        dp.Owner = accessory.Data.Me;
        dp.TargetPosition = new Vector3(100, 0, 94.5f);
        dp.ScaleMode |= ScaleMode.YByDistance;
        dp.Color = accessory.Data.DefaultSafeColor;
        dp.DestoryAt = 10000;
        accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
    ...
    [ScriptMethod(name: "P2_协同程序LB_盾连击S_男人位置连线", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:32369"])]
    ...
    [ScriptMethod(name: "P2_协同程序LB_盾连击S二段处理位置", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:31528"])]
    public void P2_协同程序LB_盾连击S二段处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        if (accessory.Data.Me == tid)
        {
            ...
            dealpos = RotatePoint(pos, new Vector3(100, 0, 100), float.Pi / 2);
        }
    ...
    [ScriptMethod(name: "P3_开场_分P", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:31507"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P3_开场_Buff收集", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(3425|3426)$"], userControl: false)]
    ...
    [ScriptMethod(name: "P3_小电视_Buff收集", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(3452|3453)$"], userControl: false)]
    ...
    [ScriptMethod(name: "P3_开场_手臂AOE", eventType: EventTypeEnum.PlayActionTimeline,
        eventCondition: ["Id:regex:^(774[78])$", "SourceDataId:regex:^(1571[89])$"], userControl: false)]
    ...
    [ScriptMethod(name: "P3_小电视_自身AOE", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3159[56])$"])]
    ...
    [ScriptMethod(name: "P3_开场_Buff预站位处理位置", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:3426"])]
    public async void P3_开场_Buff预站位处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        await Task.Delay(100);
        var sortOrder = P3_StackSort switch
        {
        ...
        if (myP3StartBuff == 2 || myP3StartBuff == 0)
            ...
            {
                1 => new Vector3(092.00f, 0, 086.14f),
                2 => new Vector3(108.00f, 0, 086.14f),
                _ => default
            };
        if (myP3StartBuff == 1)
            dealpos = myP3Index switch
            {
                1 => new Vector3(084.00f, 0, 100.00f),
                2 => new Vector3(092.00f, 0, 113.86f),
                3 => new Vector3(108.00f, 0, 113.86f),
                4 => new Vector3(116.00f, 0, 100.00f),
                _ => default
            };
        if (dealpos == default) return;
        var dp = accessory.Data.GetDefaultDrawProperties();
    ...
    [ScriptMethod(name: "P3_开场_处理位置", eventType: EventTypeEnum.PlayActionTimeline,
        eventCondition: ["Id:regex:^(774[78])$", "SourceDataId:regex:^(1571[89])$"])]
    public void P3_开场_处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        if (myP3StartBuff == 2 || myP3StartBuff == 0)
        {
            ...
            {
                1 => northCirle ? new Vector3(086.7f, 0, 086.7f) : new Vector3(094.8f, 0, 082.0f),
                2 => northCirle ? new Vector3(113.3f, 0, 086.7f) : new Vector3(105.2f, 0, 082.0f),
                _ => default
            };
            dealpos2 = myP3Index switch
            {
                1 => northCirle ? new Vector3(087.8f, 0, 087.8f) : new Vector3(095.0f, 0, 083.5f),
                2 => northCirle ? new Vector3(112.2f, 0, 087.8f) : new Vector3(105.0f, 0, 083.5f),
                _ => default
            };
            dealpos3 = myP3Index switch
            {
                1 => northCirle ? new Vector3(088.4f, 0, 085.5f) : new Vector3(093.1f, 0, 082.8f),
                2 => northCirle ? new Vector3(111.6f, 0, 085.5f) : new Vector3(106.9f, 0, 082.8f),
                _ => default
            };
            dealpos4 = myP3Index switch
            {
                1 => northCirle ? new Vector3(094.7f, 0, 083.7f) : new Vector3(088.5f, 0, 087.0f),
                2 => northCirle ? new Vector3(105.3f, 0, 083.7f) : new Vector3(111.5f, 0, 087.0f),
                _ => default
            };
        }
        ...
        if (myP3StartBuff == 1)
        {
            dealpos1 = myP3Index switch
            {
                1 => northCirle ? new Vector3(082.0f, 0, 095.0f) : new Vector3(082.0f, 0, 104.7f),
                2 => northCirle ? new Vector3(095.0f, 0, 118.0f) : new Vector3(086.5f, 0, 113.0f),
                3 => northCirle ? new Vector3(105.0f, 0, 118.0f) : new Vector3(113.5f, 0, 113.0f),
                4 => northCirle ? new Vector3(118.0f, 0, 095.0f) : new Vector3(118.0f, 0, 104.7f),
                _ => default
            };
            dealpos2 = myP3Index switch
            {
                1 => northCirle ? new Vector3(083.5f, 0, 095.5f) : new Vector3(083.5f, 0, 104.5f),
                2 => northCirle ? new Vector3(095.0f, 0, 116.5f) : new Vector3(088.0f, 0, 112.0f),
                3 => northCirle ? new Vector3(105.0f, 0, 116.5f) : new Vector3(112.0f, 0, 112.0f),
                4 => northCirle ? new Vector3(116.5f, 0, 095.5f) : new Vector3(116.5f, 0, 104.5f),
                _ => default
            };
            dealpos3 = myP3Index switch
            {
                1 => northCirle ? new Vector3(081.7f, 0, 097.2f) : new Vector3(081.6f, 0, 102.8f),
                2 => northCirle ? new Vector3(093.2f, 0, 117.2f) : new Vector3(088.5f, 0, 114.5f),
                3 => northCirle ? new Vector3(106.8f, 0, 117.2f) : new Vector3(111.5f, 0, 114.5f),
                4 => northCirle ? new Vector3(118.3f, 0, 097.2f) : new Vector3(118.4f, 0, 102.8f),
                _ => default
            };
            dealpos4 = myP3Index switch
            {
                1 => northCirle ? new Vector3(083.5f, 0, 104.0f) : new Vector3(084.0f, 0, 095.0f),
                2 => northCirle ? new Vector3(088.5f, 0, 112.5f) : new Vector3(095.0f, 0, 116.3f),
                3 => northCirle ? new Vector3(111.5f, 0, 112.5f) : new Vector3(105.0f, 0, 116.3f),
                4 => northCirle ? new Vector3(116.5f, 0, 104.0f) : new Vector3(116.0f, 0, 095.0f),
                _ => default
            };
        }
    ...
    [ScriptMethod(name: "P3_小电视_处理位置", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3159[56])$"])]
    public void P3_小电视_处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        for (var i = 0; i < HtdhParty.Count; i++)
        {
        ...
        if (P3_TV_Strategy == P3TVEnum.Normal)
        {
            if (meIsIdle)
                ...
                {
                    1 => isEast ? new Vector3(099.0f, 0, 091.0f) : new Vector3(101.0f, 0, 091.0f),
                    2 => isEast ? new Vector3(104.0f, 0, 100.0f) : new Vector3(096.0f, 0, 100.0f),
                    3 => isEast ? new Vector3(115.5f, 0, 100.0f) : new Vector3(084.5f, 0, 100.0f),
                    4 => isEast ? new Vector3(099.0f, 0, 109.0f) : new Vector3(101.0f, 0, 109.0f),
                    5 => isEast ? new Vector3(099.0f, 0, 119.0f) : new Vector3(101.0f, 0, 119.0f),
                    _ => default
                };
            else
                dealpos = myBuffIndex switch
                {
                    1 => isEast ? new Vector3(093.0f, 0, 082.0f) : new Vector3(107.0f, 0, 082.0f),
                    2 => isEast ? new Vector3(086.0f, 0, 092.5f) : new Vector3(114.0f, 0, 092.5f),
                    3 => isEast ? new Vector3(086.0f, 0, 107.5f) : new Vector3(114.0f, 0, 107.5f),
                    _ => default
                };
        }
        ...
        if (P3_TV_Strategy == P3TVEnum.Static)
        {
            if (meIsIdle)
                ...
                {
                    1 => isEast ? new Vector3(099.0f, 0, 091.0f) : new Vector3(101.0f, 0, 091.0f),
                    2 => new Vector3(109.0f, 0, 100.0f),
                    3 => new Vector3(119.0f, 0, 100.0f),
                    4 => isEast ? new Vector3(099.0f, 0, 109.0f) : new Vector3(101.0f, 0, 109.0f),
                    5 => isEast ? new Vector3(099.0f, 0, 119.0f) : new Vector3(101.0f, 0, 119.0f),
                    _ => default
                };
                ...
                dealpos = myBuffIndex switch
                {
                    1 => isEast ? new Vector3(095.0f, 0, 082.0f) : new Vector3(105.0f, 0, 082.0f),
                    2 => new Vector3(086.0f, 0, 092.0f),
                    3 => new Vector3(086.0f, 0, 108.0f),
                    _ => default
                };
        }
    ...
    [ScriptMethod(name: "P3_小电视_面向辅助", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3159[56])$"])]
    ...
    [ScriptMethod(name: "P4_分P", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:31559"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P4_分摊点名记录", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:22393"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P4_第一段波动炮命中提示", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:31614", "TargetIndex:1"])]
    ...
    [ScriptMethod(name: "P4_第一段波动炮引导位置", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:regex:^(3161[07])$"])]
    public void P4_第一段波动炮引导位置(Event @event, ScriptAccessory accessory)
    {
        if (parse != 4.0) return;
        var myindex = accessory.Data.PartyList.IndexOf(accessory.Data.Me);
        var dealpos = myindex switch
        {
            0 => new Vector3(087.5f, 0, 094.5f),
            6 => new Vector3(086.5f, 0, 100.0f),
            2 => new Vector3(087.5f, 0, 105.0f),
            4 => new Vector3(090.5f, 0, 109.5f),
            1 => new Vector3(112.5f, 0, 094.5f),
            7 => new Vector3(113.5f, 0, 100.0f),
            3 => new Vector3(112.5f, 0, 105.0f),
            5 => new Vector3(109.5f, 0, 109.5f),
            _ => default
        };
        if (dealpos == default) return;
    ...
    [ScriptMethod(name: "P4_第二段波动炮分摊位置", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:31614", "TargetIndex:1"])]
    public void P4_第二段波动炮分摊位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var dealpos = leftGroup.Contains(myindex) ? new Vector3(96.5f, 0, 113) : new Vector3(103.5f, 0, 113);
    ...
    [ScriptMethod(name: "P5_一运_分P", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:31624"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P5_一运_眼睛记录", eventType: EventTypeEnum.EnvControl, eventCondition: ["Id:00020001"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P5_一运_Buff记录", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(3442|3443|3440|3504)$"], userControl: false)]
    ...
    [ScriptMethod(name: "P5_一运_连线记录", eventType: EventTypeEnum.Tether, eventCondition: ["Id:regex:^(00C9|00C8)$"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P5_一运_飞拳记录", eventType: EventTypeEnum.AddCombatant,
        eventCondition: ["DataId:regex:^(15709|15710)$"], userControl: false)]
    public void P5_一运_飞拳记录(Event @event, ScriptAccessory accessory)
    {
        if (parse != 5.1) return;
        var pos = JsonConvert.DeserializeObject<Vector3>(@event["SourcePosition"]);
        var rot = FloorPositionTo4Dir(pos, new Vector3(100, 0, 100));
        lock (P51_Fist)
        {
    ...
    public async void P5_一运_拉线预站位位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var myindex = accessory.Data.PartyList.IndexOf(accessory.Data.Me);
        var dealpos = StartList.IndexOf(myindex) switch
        {
        ...
        dealpos = RotatePoint(dealpos, new Vector3(100, 0, 100), float.Pi / 4 * P51_Eye);
    ...
    [ScriptMethod(name: "P5_一运_飞拳站位", eventType: EventTypeEnum.AddCombatant,
        eventCondition: ["DataId:regex:^(15709)$"])]
