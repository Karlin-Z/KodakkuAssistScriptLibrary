...
public class EdenUltimate
{
    ...
    private readonly List<int> P2DDIceDir = [];
    private readonly List<int> P2LightRampantCircle = [];
    private readonly List<int> P2RedMirror = [];
    private List<int> P1四连线 = [];
    private bool P1四连线开始;
    private List<int> P1塔 = [0, 0, 0, 0];
    private List<int> P1转轮召抓人 = [0, 0, 0, 0, 0, 0, 0, 0];

    private bool P1转轮召雷;

    private int P1雾龙计数;
    private List<int> P1雾龙记录 = [0, 0, 0, 0];
    private bool P1雾龙雷;
    private uint P2BossId;

    private bool P2DDDircle;
    private List<int> P2LightRampantBuff = [0, 0, 0, 0, 0, 0, 0, 0];
    private bool P2LightRampantTetherDone;

    private List<int> P3FireBuff = [0, 0, 0, 0, 0, 0, 0, 0];
    private int P3FloorFire;
    private bool P3FloorFireDone;
    private List<int> P3Lamp = [0, 0, 0, 0, 0, 0, 0, 0];
    private List<int> P3LampWise = [0, 0, 0, 0, 0, 0, 0, 0];
    private List<int> P3ReturnBuff = [0, 0, 0, 0, 0, 0, 0, 0];
    private List<int> P3Stack = [0, 0, 0, 0, 0, 0, 0, 0];
    private List<int> P3WaterBuff = [0, 0, 0, 0, 0, 0, 0, 0];
    private int P4BlueTether;
    private List<int> P4ClawBuff = [0, 0, 0, 0, 0, 0, 0, 0];

    private uint P4FragmentId;
    private List<int> P4OtherBuff = [0, 0, 0, 0, 0, 0, 0, 0];
    private List<int> P4Stack = [0, 0, 0, 0, 0, 0, 0, 0];
    private List<int> P4Tether = [-1, -1, -1, -1, -1, -1, -1, -1];
    private bool P4TetherDone;
    private List<Vector3> P4WaterPos = [];
    private List<Vector3> P4WhiteCirclePos = [];

    private bool P5MtDone;
    private string P5Tower = "";

    private int? firstTargetIcon;
    private double parse;

    [UserSetting("P1_转轮召分组依据")] public P1BrightFireEnum P1BrightFireGroup { get; set; }

    [UserSetting("P1_四连线头顶标记")] public P1TetherEnum p1Thther4Type { get; set; }

    [UserSetting("P1_四连线头顶标记")] public bool p1Thther4Marker { get; set; } = false;

    [UserSetting("P2_光爆拉线方式")] public P2LightRampantTetherEmum P2LightRampantTetherDeal { get; set; }

    [UserSetting("P2_光爆八方站位方式")] public P2LightRampant8DirEmum P2LightRampant8DirSet { get; set; }

    [UserSetting("P3_分灯方式")] public P3LampEmum P3LampDeal { get; set; }

    [UserSetting("P3_T跳远引导位置")] public bool P3JumpPosition { get; set; } = false;

    [UserSetting("P4_二运常/慢灯AOE显示时间(ms)")] public uint P4LampDisplayDur { get; set; } = 3000;

    [UserSetting("P4_白圈提示线颜色")] public ScriptColor P4WhiteCircleLineColor { get; set; } = new();

    [UserSetting("P5_地火颜色")] public ScriptColor P5PathColor { get; set; } = new() { V4 = new Vector4(0, 1, 1, 1) };
    ...
    {
        firstTargetIcon ??= int.Parse(id, NumberStyles.HexNumber);
        return int.Parse(id, NumberStyles.HexNumber) - (int)firstTargetIcon;
    }
    ...
    [ScriptMethod(name: "P1_八方雷火_引导扇形", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^((4014[48])|40329|40330)$"])]
    ...
    [ScriptMethod(name: "P1_八方雷火_后续扇形", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:regex:^(40145)$", "TargetIndex:1"])]
    ...
    [ScriptMethod(name: "P1_八方雷火_分散分摊", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^((4014[48])|40329|40330)$"])]
    ...
    [ScriptMethod(name: "P1_八方雷火_引导位置", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^((4014[48])|40329|40330)$"])]
    public void P1_八方雷火_引导位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var mPosEnd = RotatePoint(outPoint ? new Vector3(100, 0, 90) : new Vector3(100, 0, 95),
            new Vector3(100, 0, 100), float.Pi / 4 * rot8);
    ...
    [ScriptMethod(name: "P1_雾龙_位置记录", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(40158)$"], userControl: false)]
    public void P1_雾龙_位置记录(Event @event, ScriptAccessory accessory)
    {
        ...
        if (obj == null) return;
        var dir8 = PositionTo8Dir(obj.Position, new Vector3(100, 0, 100));
        P1雾龙记录[dir8 % 4] = 1;
    }

    [ScriptMethod(name: "P1_雾龙_雷火记录", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:regex:^(4015[45])$"], userControl: false)]
    ...
    [ScriptMethod(name: "P1_雾龙_范围", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(40158)$"])]
    ...
    [ScriptMethod(name: "P1_雾龙_分散分摊", eventType: EventTypeEnum.ActionEffect,
    ...
    [ScriptMethod(name: "P1_雾龙_预站位位置", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4015[45])$"])]
    public void P1_雾龙_预站位位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var mPosEnd = RotatePoint(new Vector3(100, 0, 82), new Vector3(100, 0, 100), float.Pi / 4 * rot8);
        if (myindex == 0) mPosEnd = RotatePoint(mPosEnd, new Vector3(100, 0, 100), float.Pi / 36);
        if (myindex == 6) mPosEnd = RotatePoint(mPosEnd, new Vector3(100, 0, 100), float.Pi / -36);
    ...
    [ScriptMethod(name: "P1_雾龙_处理位置", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(40158)$"])]
        ...
        lock (this)
        {
            ...
            Task.Delay(100).ContinueWith(t =>
            {
                if (!P1雾龙雷)
                {
                    ...
                    var mPosEnd = RotatePoint(new Vector3(100, 0, 84), new Vector3(100, 0, 100), float.Pi / 4 * rot8);
                ...
                else
                {
                    ...
                    var mPosEnd = RotatePoint(myPosA, new Vector3(100, 0, 100), float.Pi / 4 * rot8);
    ...
    [ScriptMethod(name: "P1_转轮召_雷火记录", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4015[01])$"], userControl: false)]
    ...
    [ScriptMethod(name: "P1_转轮召_雷直线", eventType: EventTypeEnum.StartCasting,
    ...
    [ScriptMethod(name: "P1_转轮召_火直线", eventType: EventTypeEnum.StartCasting,
    ...
    [ScriptMethod(name: "P1_转轮召_抓人记录", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:4165"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P1_转轮召_击退处理位置", eventType: EventTypeEnum.StartCasting,
    ...
    [ScriptMethod(name: "P1_四连线_清除连线记录器", eventType: EventTypeEnum.StartCasting,
    ...
    [ScriptMethod(name: "P1_四连线_连线记录器", eventType: EventTypeEnum.Tether, eventCondition: ["Id:regex:^(00F9|011F)$"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P1_四连线_头顶标记", eventType: EventTypeEnum.Tether, eventCondition: ["Id:regex:^(00F9|011F)$"],
        userControl: false)]
    public void P1_四连线_头顶标记(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(50).ContinueWith(t =>
        {
            ...
            accessory.Method.Mark(accessory.Data.PartyList[index], (MarkType)P1四连线.Count);
    ...
    public void P1_四连线_处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(50).ContinueWith(t =>
        {
            ...
            if (P1四连线.Count == 4)
            {
                ...
                var i1p1 = tehterIsFire[0]
                    ? new Vector3(100, 0, 100 - far - dis)
                    : new Vector3(100 - dis, 0, 100 - far);
                var i1p2 = tehterIsFire[2]
                    ? new Vector3(100, 0, 100 - far - dis)
                    : new Vector3(100 - dis, 0, 100 - far);
                var i2p1 = tehterIsFire[0]
                    ? new Vector3(100, 0, 100 - far - dis)
                    : new Vector3(100 + dis, 0, 100 - far);
                var i2p2 = tehterIsFire[2]
                    ? new Vector3(100, 0, 100 - far - dis)
                    : new Vector3(100 + dis, 0, 100 - far);
                var i3p1 = tehterIsFire[1]
                    ? new Vector3(100, 0, 100 + far + dis)
                    : new Vector3(100 - dis, 0, 100 + far);
                var i3p2 = tehterIsFire[3]
                    ? new Vector3(100, 0, 100 + far + dis)
                    : new Vector3(100 - dis, 0, 100 + far);
                var i4p1 = tehterIsFire[1]
                    ? new Vector3(100, 0, 100 + far + dis)
                    : new Vector3(100 + dis, 0, 100 + far);
                var i4p2 = tehterIsFire[3]
                    ? new Vector3(100, 0, 100 + far + dis)
                    : new Vector3(100 + dis, 0, 100 + far);
    ...
    [ScriptMethod(name: "P1_塔_塔记录器", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4012[234567]|4013[15])$"], userControl: false)]
    public void P1_塔_塔记录器(Event @event, ScriptAccessory accessory)
    {
        ...
        lock (this)
        {
            ...
            if (pos.X - 100 > 1) P1塔[3] = 1;
        }
    }

    [ScriptMethod(name: "P1_塔_雷火直线", eventType: EventTypeEnum.StartCasting,
    ...
    [ScriptMethod(name: "P1_塔_塔处理位置", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(40134|40129)$"])]
        ...
        Task.Delay(100).ContinueWith(t =>
        {
            ...
            if (@event["ActionId"] == "40134")
            {
                ...
                if (myindex == 0 || myindex == 1)
                {
                    ...
                    dp.Scale = new Vector2(2);
                    dp.Owner = accessory.Data.Me;
                    dp.TargetPosition = new Vector3(100 + dx, 0, 100 + dy);
                    dp.ScaleMode |= ScaleMode.YByDistance;
                    dp.Color = accessory.Data.DefaultSafeColor;
                    dp.DestoryAt = 10500;
                    accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
                ...
                else
                {
                    ...
                    if (myIndex2 > 0 && myIndex2 <= P1塔[0])
                        dealpos = new Vector3(eastTower ? 113.08f : 86.92f, 0, 90.81f);
                    if (myIndex2 > P1塔[0] && myIndex2 <= P1塔[0] + P1塔[1])
                        dealpos = new Vector3(eastTower ? 115.98f : 84.02f, 0, 100f);
                    if (myIndex2 > P1塔[0] + P1塔[1] && myIndex2 <= P1塔[0] + P1塔[1] + P1塔[2])
                        dealpos = new Vector3(eastTower ? 113.08f : 86.92f, 0, 109.18f);

                    Vector3 towerpos = default;
                    if (myIndex2 > 0 && myIndex2 <= P1塔[0])
                        towerpos = new Vector3(eastTower ? 113.08f : 86.92f, 0, 90.81f);
                    if (myIndex2 > P1塔[0] && myIndex2 <= P1塔[0] + P1塔[1])
                        towerpos = new Vector3(eastTower ? 115.98f : 84.02f, 0, 100f);
                    if (myIndex2 > P1塔[0] + P1塔[1] && myIndex2 <= P1塔[0] + P1塔[1] + P1塔[2])
                        towerpos = new Vector3(eastTower ? 113.08f : 86.92f, 0, 109.18f);
            ...
            else
            {
                ...
                if (myindex == 0 || myindex == 1)
                {
                    ...
                    dp.Scale = new Vector2(2);
                    dp.Owner = accessory.Data.Me;
                    dp.TargetPosition = new Vector3(100 + dx1, 0, 100 + dy);
                    dp.ScaleMode |= ScaleMode.YByDistance;
                    dp.Color = accessory.Data.DefaultSafeColor;
                    dp.DestoryAt = 6500;
                    accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
                    ...
                    dp.Scale = new Vector2(2);
                    dp.Position = new Vector3(100 + dx1, 0, 100 + dy);
                    dp.TargetPosition = new Vector3(100 + dx2, 0, 100 + dy);
                    dp.ScaleMode |= ScaleMode.YByDistance;
                    dp.Color = accessory.Data.DefaultSafeColor;
                    dp.DestoryAt = 6500;
                    accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
                ...
                else
                {
                    var myIndex2 = myindex - 1;
                    Vector3 dealpos = default;
                    if (myIndex2 > 0 && myIndex2 <= P1塔[0]) dealpos = new Vector3(eastTower ? 102f : 98f, 0, 90.81f);
                    if (myIndex2 > P1塔[0] && myIndex2 <= P1塔[0] + P1塔[1])
                        dealpos = new Vector3(eastTower ? 102f : 98f, 0, 100f);
                    if (myIndex2 > P1塔[0] + P1塔[1] && myIndex2 <= P1塔[0] + P1塔[1] + P1塔[2])
                        dealpos = new Vector3(eastTower ? 102f : 98f, 0, 109.18f);
    ...
    [ScriptMethod(name: "P2_换P", eventType: EventTypeEnum.Director,
        eventCondition: ["Instance:800375BF", "Command:8000001E"], userControl: false)]
    public void P2_换P(Event @event, ScriptAccessory accessory)
    {
        parse = 2d;
    }

    [ScriptMethod(name: "P2_钻石星尘_BossId记录", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(40180)$"], userControl: false)]
    ...
    [ScriptMethod(name: "P2_钻石星尘_钢铁月环记录", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^((4020[23]))$"], userControl: false)]
    public void P2_钻石星尘_钢铁月环记录(Event @event, ScriptAccessory accessory)
    {
        P2DDDircle = @event["ActionId"] == "40202"; //钢铁
    }

    [ScriptMethod(name: "P2_钻石星尘_钢铁月环", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^((4020[23]))$"])]
    ...
    [ScriptMethod(name: "P2_钻石星尘_扇形引导", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^((4020[23]))$"])]
    ...
    public void P2_钻石星尘_冰花放置位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var epos1 = P2DDDircle ? new Vector3(119.5f, 0, 100.0f) : new Vector3(103.5f, 0, 100.0f);
        var epos2 = P2DDDircle ? new Vector3(119.5f, 0, 100.0f) : new Vector3(108.0f, 0, 100.0f);
        var dir8 = P2DDIceDir.FirstOrDefault() % 4;
        var dr = dir8 == 0 || dir8 == 2 ? -1 : 0;
        var dealpos1 = RotatePoint(epos1, new Vector3(100, 0, 100), float.Pi / 4 * (rot + dr));
        var dealpos2 = RotatePoint(epos2, new Vector3(100, 0, 100), float.Pi / 4 * (rot + dr));
        var dp = accessory.Data.GetDefaultDrawProperties();
    ...
    public void P2_钻石星尘_扇形引导位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var epos = P2DDDircle ? new Vector3(116.5f, 0, 100f) : new Vector3(101f, 0, 100f);
        var dealpos = RotatePoint(epos, new Vector3(100, 0, 100), float.Pi / 4 * (rot + dr));
        var dp = accessory.Data.GetDefaultDrawProperties();
    ...
    [ScriptMethod(name: "P2_钻石星尘_九连环记录", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(40198)$"], userControl: false)]
    public void P2_钻石星尘_九连环记录(Event @event, ScriptAccessory accessory)
    {
        ...
        lock (P2DDIceDir)
        {
            P2DDIceDir.Add(PositionTo8Dir(pos, new Vector3(100, 0, 100)));
        }
    }

    [ScriptMethod(name: "P2_钻石星尘_击退位置", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:regex:^((4020[23]))$", "TargetIndex:1"])]
    public void P2_钻石星尘_击退位置(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(2500).ContinueWith(t =>
        {
            ...
            rot += isH1Group ? 4 : 0;
            var dealpos = RotatePoint(nPos, new Vector3(100, 0, 100), float.Pi / 4 * rot);
            var dp = accessory.Data.GetDefaultDrawProperties();
    ...
    [ScriptMethod(name: "P2_钻石星尘_连续剑分身位置", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:regex:^40208$", "TargetIndex:1"])]
    ...
    [ScriptMethod(name: "P2_钻石星尘_连续剑范围", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^4019[34]$"])]
    ...
    [ScriptMethod(name: "P2_钻石星尘_Boss背对", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:regex:^40208$", "TargetIndex:1"])]
    ...
    [ScriptMethod(name: "P2_双镜_分P", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(40179)$"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P2_双镜_分散分摊", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4022[01])$"])]
    public void P2_双镜_分散分摊(Event @event, ScriptAccessory accessory)
    {
        ...
        else
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                ...
                dp.Scale = new Vector2(5);
                dp.Owner = accessory.Data.PartyList[i];
                dp.Color = group[myindex] == i || i == myindex
    ...
    [ScriptMethod(name: "P2_双镜_蓝镜月环加引导", eventType: EventTypeEnum.EnvControl,
    ...
    public void P2_双镜_蓝镜月环加引导(Event @event, ScriptAccessory accessory)
    {
        ...
        dir8--;
        var dealpos = RotatePoint(npos, new Vector3(100, 0, 100), float.Pi / 4 * dir8);
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = "P2_双镜_蓝镜月环";
        dp.Scale = new Vector2(20);
        dp.InnerScale = new Vector2(4);
    ...
    [ScriptMethod(name: "P2_双镜_红镜月环加引导", eventType: EventTypeEnum.EnvControl,
    ...
    [ScriptMethod(name: "P2_双镜_蓝镜月环加引导位置", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["DirectorId:800375BF", "State:00020001"])]
    ...
    [ScriptMethod(name: "P2_双镜_红镜引导位置", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["DirectorId:800375BF", "State:02000100"])]
    public void P2_双镜_红镜引导位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var leftRot8 = P2RedMirror[0] - P2RedMirror[1] == -2 || P2RedMirror[0] - P2RedMirror[1] - 8 == -2
            ? P2RedMirror[0]
            : P2RedMirror[1];
        var rightRot8 = P2RedMirror[0] - P2RedMirror[1] == 2 || P2RedMirror[0] + 8 - P2RedMirror[1] == 2
            ? P2RedMirror[0]
            : P2RedMirror[1];

        var myrot = leftRot8;
        var myindex = accessory.Data.PartyList.IndexOf(accessory.Data.Me);
        if (myindex == 0 || myindex == 1 || myindex == 4 || myindex == 5) myrot = rightRot8;
        var npos = myindex switch
        {
            0 => new Vector3(102f, 0, 80.5f),
            1 => new Vector3(98f, 0, 80.5f),
            2 => new Vector3(102f, 0, 80.5f),
            3 => new Vector3(98f, 0, 80.5f),
            4 => new Vector3(101.3f, 0, 83f),
            5 => new Vector3(98.7f, 0, 83f),
            6 => new Vector3(101.3f, 0, 83f),
            7 => new Vector3(98.7f, 0, 83f),
            _ => new Vector3(100, 0, 80)
        };
        var dealpos = RotatePoint(npos, new Vector3(100, 0, 100), myrot * float.Pi / 4);
        var dp = accessory.Data.GetDefaultDrawProperties();
    ...
    [ScriptMethod(name: "P2_光之暴走_分P", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(40212)$"], userControl: false)]
    ...
    [ScriptMethod(name: "P2_光之暴走_Buff收集", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:2257"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P2_光之暴走_分散分摊", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4022[01])$"])]
    ...
    public void P2_光之暴走_塔处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(50).ContinueWith(t =>
        {
            ...
            {
                var count = 0;
                if (myindex == 0) dealpos = t4;
                count += P2LightRampantCircle.Contains(0) ? 1 : 0;
                if (myindex == 7) dealpos = P2LightRampantCircle.Contains(0) ? t4 : t2;
                ...
                if (myindex == 2) dealpos = P2LightRampantCircle.Contains(6) ? t1 : t5;
                if (myindex == 6) dealpos = t1;
            ...
            if (P2LightRampantTetherDeal == P2LightRampantTetherEmum.NewGrey9)
            {
                ...
                if (myindex == 0) dealpos = t4;
                count += P2LightRampantCircle.Contains(0) ? 1 : 0;
                if (myindex == 7) dealpos = P2LightRampantCircle.Contains(0) ? t4 : t6;
                count += P2LightRampantCircle.Contains(7) ? 1 : 0;
                if (myindex == 1)
                {
                ...
                count += P2LightRampantCircle.Contains(4) ? 1 : 0;
                if (myindex == 2) dealpos = P2LightRampantCircle.Contains(6) ? t1 : t3;
                if (myindex == 6) dealpos = t1;
    ...
    [ScriptMethod(name: "P2_双镜_中央踩塔位置", eventType: EventTypeEnum.EnvControl,
    ...
    [ScriptMethod(name: "P2_光之暴走_八方分散位置", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:regex:^(4022[01])$"])]
    public void P2_光之暴走_八方分散位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var mPosEnd = RotatePoint(new Vector3(100, 0, 95), new Vector3(100, 0, 100), float.Pi / 4 * rot8);
    ...
    [ScriptMethod(name: "P3_时间压缩_分P", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(40266)$"], userControl: false)]
    ...
    [ScriptMethod(name: "P3_时间压缩_Buff记录", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(2455|2456|2464|2462|2461|2460)$"], userControl: false)]
    ...
    [ScriptMethod(name: "P3_时间压缩_灯记录", eventType: EventTypeEnum.Tether, eventCondition: ["Id:regex:^(0085|0086)$"],
        userControl: false)]
    public void P3_时间压缩_灯记录(Event @event, ScriptAccessory accessory)
    {
        ...
        if (parse != 3.1) return;
        var pos = JsonConvert.DeserializeObject<Vector3>(@event["SourcePosition"]);
        var dir8 = PositionTo8Dir(pos, new Vector3(100, 0, 100));
        lock (P3Lamp)
        {
    ...
    [ScriptMethod(name: "P3_时间压缩_灯顺逆记录", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:2970"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P3_时间压缩_灯AOE", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:40235", "TargetIndex:1"])]
    ...
    public void P3_时间压缩_Buff处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var returnPosN = P3WaterBuff[myIndex] == 2 ? new Vector3(100, 0, 91.5f) : new Vector3(100, 0, 98);
    ...
    public void P3_时间压缩_灯处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var myIndex = accessory.Data.PartyList.IndexOf(accessory.Data.Me);
        var dir8 = PositionTo8Dir(pos, centre);
        var nPos = @event["StackCount"] == "92" ? new Vector3(98, 0, 90) : new Vector3(102, 0, 90);
    ...
    [ScriptMethod(name: "P3_延迟咏唱回响_分P", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(40269)$"], userControl: false)]
    ...
    [ScriptMethod(name: "P3_延迟咏唱回响_地火", eventType: EventTypeEnum.ObjectEffect,
    ...
    [ScriptMethod(name: "P3_延迟咏唱回响_击退提示", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:40182", "TargetIndex:1"])]
    ...
    [ScriptMethod(name: "P3_延迟咏唱回响_地火记录", eventType: EventTypeEnum.ObjectEffect,
        eventCondition: ["Id1:4", "Id2:regex:^(16|64)$"], userControl: false)]
    public void P3_延迟咏唱回响_地火记录(Event @event, ScriptAccessory accessory)
    {
        ...
        lock (this)
        {
            ...
            Vector3 centre = new(100, 0, 100);
            var pos = JsonConvert.DeserializeObject<Vector3>(@event["SourcePosition"]);
            P3FloorFire = PositionTo8Dir(pos, new Vector3(100, 0, 100));
    ...
    public void P3_延迟咏唱回响_T引导位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var pos1 = RotatePoint(posN, new Vector3(100, 0, 100), float.Pi / 4 * rot);
        var pos2 = RotatePoint(posN, new Vector3(100, 0, 100), float.Pi / 4 * rot + float.Pi);
    ...
    [ScriptMethod(name: "P4_具象化_分P", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:40246"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P4_时间结晶_记忆水晶收集", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:40174"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P4_暗光龙诗_分P", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:40239"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P4_暗光龙诗_Buff记录", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:2461"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P4_暗光龙诗_连线收集", eventType: EventTypeEnum.Tether, eventCondition: ["Id:006E"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P4_暗光龙诗_神圣之翼", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4022[78])$"])]
    ...
    [ScriptMethod(name: "P4_暗光龙诗_水分摊", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4022[78])$"])]
    public void P4_暗光龙诗_水分摊(Event @event, ScriptAccessory accessory)
    {
        ...
        // t连线 高d 低d 蝴蝶结
        if ((P4Tether[tIndex] == d1Index && P4Tether[d2Index] == tIndex) ||
            (P4Tether[tIndex] == d2Index && P4Tether[d1Index] == tIndex))
        {
            upGroup.Add(tIndex);
            upGroup.Add(nIndex);
        ...
        // t连线 高d n 方块
        if ((P4Tether[tIndex] == d1Index && P4Tether[nIndex] == tIndex) ||
            (P4Tether[d1Index] == tIndex && P4Tether[tIndex] == nIndex))
        {
            upGroup.Add(d1Index);
            upGroup.Add(nIndex);
        ...
        // t连线 低d n 沙漏
        if ((P4Tether[tIndex] == d2Index && P4Tether[nIndex] == tIndex) ||
            (P4Tether[d2Index] == tIndex && P4Tether[tIndex] == nIndex))
        {
            upGroup.Add(tIndex);
            upGroup.Add(d1Index);
        ...
        dp.Color = upGroup.Contains(tetherStack) == upGroup.Contains(myindex)
            ? accessory.Data.DefaultSafeColor
            : accessory.Data.DefaultDangerColor;
        dp.DestoryAt = 5000;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
        ...
        dp.Color = upGroup.Contains(idleStack) == upGroup.Contains(myindex)
            ? accessory.Data.DefaultSafeColor
            : accessory.Data.DefaultDangerColor;
        dp.DestoryAt = 5000;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    ...
    public void P4_暗光龙诗_塔处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(50).ContinueWith(t =>
        {
            ...
            var myIndex = accessory.Data.PartyList.IndexOf(accessory.Data.Me);
            var dealpos = upGroup.Contains(myIndex) ? new Vector3(100, 0, 92) : new Vector3(100, 0, 108);
    ...
    public void P4_暗光龙诗_引导处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(50).ContinueWith(t =>
        {
            ...
            var dealpos = idles.IndexOf(myIndex) switch
            {
                0 => new Vector3(095.8f, 0, 098.0f),
                1 => new Vector3(104.2f, 0, 098.0f),
                2 => new Vector3(095.8f, 0, 102.0f),
                3 => new Vector3(104.2f, 0, 102.0f)
            };
    ...
    [ScriptMethod(name: "P4_暗光龙诗_分摊处理位置", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(4022[78])$"])]
    ...
    [ScriptMethod(name: "P4_时间结晶_分P", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:40240"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P4_时间结晶_Buff收集", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(326[34]|2454|246[0123])$"], userControl: false)]
    public void P4_时间结晶_Buff收集(Event @event, ScriptAccessory accessory)
    {
        ...
        if (id == "3264") P4ClawBuff[index] = 3;
        //暗 4
        if (id == "2460") P4OtherBuff[index] = 4;
        //水 3
        if (id == "2461") P4OtherBuff[index] = 3;
        //冰 1
        if (id == "2462") P4OtherBuff[index] = 1;
        //风 2
        if (id == "2463") P4OtherBuff[index] = 2;
        //土 5
        if (id == "2454") P4OtherBuff[index] = 5;
    }

    [ScriptMethod(name: "P4_时间结晶_蓝线收集", eventType: EventTypeEnum.Tether, eventCondition: ["Id:0085"],
        userControl: false)]
    public void P4_时间结晶_蓝线收集(Event @event, ScriptAccessory accessory)
    {
        if (parse != 4.3) return;
        var pos = JsonConvert.DeserializeObject<Vector3>(@event["SourcePosition"]);
        P4BlueTether = PositionTo6Dir(pos, new Vector3(100, 0, 100)) % 3;
    }
    ...
    public void P4_时间结晶_Buff处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        if (P4ClawBuff[myIndex] == 1)
        {
            ...
            var dealpos = isHigh ? new Vector3(87, 0, 100) : new Vector3(113, 0, 100);
            ...
            //低分摊 112 085 -> 107 082
            //低闲   119 103 -> 119 97
            var dealpos2 = isHigh ? P4BlueTether == 1 ? new Vector3(081, 0, 103) : new Vector3(088, 0, 085) :
                P4BlueTether == 1 ? new Vector3(112, 0, 085) : new Vector3(119, 0, 103);
            var dealpos3 = isHigh ? P4BlueTether == 1 ? new Vector3(081, 0, 097) : new Vector3(093, 0, 082) :
                P4BlueTether == 1 ? new Vector3(107, 0, 082) : new Vector3(119, 0, 097);
        ...
        if (P4ClawBuff[myIndex] == 2)
        {
            ...
            var dealpos1 = isHigh ? new Vector3(088.5f, 0, 115.5f) : new Vector3(111.5f, 0, 115.5f);
            var dealpos2 = isHigh ? new Vector3(090.2f, 0, 117.0f) : new Vector3(109.8f, 0, 117.0f);
            var dealpos3 = isHigh ? new Vector3(092.5f, 0, 118.0f) : new Vector3(107.5f, 0, 118.0f);
            var dealpos4 = isHigh ? new Vector3(092.0f, 0, 110.0f) : new Vector3(108.0f, 0, 110.0f);
        ...
        if (P4ClawBuff[myIndex] == 3)
        {
            ...
            {
                var dealpos1 = P4BlueTether == 1 ? new Vector3(112, 0, 85) : new Vector3(88, 0, 85);
                var dealpos2 = P4BlueTether == 1 ? new Vector3(108.7f, 0, 85) : new Vector3(091.3f, 0, 85);
                var dp = accessory.Data.GetDefaultDrawProperties();
            ...
            {
                var dealpos1 = P4BlueTether == 1 ? new Vector3(88, 0, 115) : new Vector3(112, 0, 115);
                var dealpos2 = P4BlueTether == 1 ? new Vector3(090.8f, 0, 116.0f) : new Vector3(109.2f, 0, 116.0f);
                var dp = accessory.Data.GetDefaultDrawProperties();
    ...
    [ScriptMethod(name: "P4_时间结晶_白圈位置指示", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:40241", "TargetIndex:1"])]
    ...
    [ScriptMethod(name: "P5_地火", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(40118|40307)$"])]
    ...
    [ScriptMethod(name: "P5_地火消除", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:regex:^(40118|4030[789])$"], userControl: false)]
    public void P5_地火消除(Event @event, ScriptAccessory accessory)
    {
        ...
        if ((posNext - centre).Length() > 20) accessory.Method.RemoveDraw($"P5_地火_前进_{@event["SourceId"]}");
    }

    [ScriptMethod(name: "P5_光与暗之翼", eventType: EventTypeEnum.StartCasting,
    ...
    public void P5_光与暗之翼(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Scale = new Vector2(4);
        dp.Owner = sid;
        dp.CentreResolvePattern = @event["ActionId"] == "40313"
    ...
    [ScriptMethod(name: "P5_光与暗之翼_塔收集", eventType: EventTypeEnum.EnvControl,
    ...
    [ScriptMethod(name: "P5_光与暗之翼_MT引导位置", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["DirectorId:800375BF", "State:00010004", "Index:regex:^(0000003[012])"])]
    public void P5_光与暗之翼_MT引导位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var towerPos = P5Tower switch
        {
            "00000032" => new Vector3(100, 0, 107),
            "00000031" => new Vector3(106.06f, 0, 96.50f),
            "00000030" => new Vector3(93.94f, 0, 96.50f)
        };

        var mtPos1 = RotatePoint(towerPos, new Vector3(100, 0, 100), float.Pi);
        var dp = accessory.Data.GetDefaultDrawProperties();
    ...
    [ScriptMethod(name: "P5_光与暗之翼_T引导位置", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(40313|40233)$"])]
    public void P5_光与暗之翼_T引导位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var mtPos1 = RotatePoint(towerPos, new Vector3(100, 0, 100), light ? mtRot : -mtRot);
        var mtPos2 = light
            ? new Vector3((mtPos1.X - 100) / 7 + 100, 0, (mtPos1.Z - 100) / 7 + 100)
            : new Vector3((mtPos1.X - 100) / 7 * 15 + 100, 0, (mtPos1.Z - 100) / 7 * 15 + 100);
        var stRot = 105f / 180 * MathF.PI;
        var stPos2 = RotatePoint(mtPos1, new Vector3(100, 0, 100), light ? stRot : -stRot);
        var stPos1 = light
            ? new Vector3((stPos2.X - 100) / 7 * 15 + 100, 0, (stPos2.Z - 100) / 7 * 15 + 100)
            : new Vector3((stPos2.X - 100) / 7 + 100, 0, (stPos2.Z - 100) / 7 + 100);
        var myindex = accessory.Data.PartyList.IndexOf(accessory.Data.Me);
        if (myindex == 0)
        {
            var dp = accessory.Data.GetDefaultDrawProperties();
