...
[ScriptType(name: "绝龙诗绘图", territorys: [968], guid: "d9c97e91-9b59-432d-a3a1-42a8475b7e2a", version: "0.0.0.6",
    author: "Karlin")]
public class DragongSingDraw
{
    private readonly object lockObj = new();
    private uint darkDragonId;

    private int? firstTargetIcon;
    private uint p1AdelId;

    private bool p1Charge;
    private uint p1GrenoId;
    private List<int> p1sony = [];
    private Vector3 p2AdelPos = Vector3.Zero;
    private List<uint> p2BlueCircle = [];

    private (int, int) p2Jump = (-1, -1);
    private List<bool> p2SafeDir = [];
    private List<bool> p2Stone = [];
    private (int, int) p2StoneMem = (-1, -1);
    private List<int> p2StoneTeam = [];
    private List<bool> p2Tower = [];
    private Vector3 p2ZPos = Vector3.Zero;
    private List<bool> p3Boom = [];
    private uint p3BossId;
    private List<int> p3Tower = [];
    private bool p3TowerDeal;
    private Dictionary<string, HashSet<uint>> p3majong = new();
    private bool p5Deal;
    private Vector3 p5DivePos = Vector3.Zero;
    private Vector3 p5GreekPos = Vector3.Zero;
    private Vector3 p5GrenoPos = Vector3.Zero;
    private List<int> p5sony = [];
    private uint p6FireBallCount;
    private uint p6FireBallCount2;
    private Vector3 p6WhitePos = Vector3.Zero;
    private List<int> p6lightDark = [];
    private List<int> p6tether = [];
    private Vector3 p7Stone1 = Vector3.Zero;
    private Vector3 p7Stone2 = Vector3.Zero;

    private double parse;
    private uint tordanId;
    private uint whiteDragonId;

    [UserSetting("P5 一运连线冲锋显示延迟(ms)")] public int p5TetherCrashDelay { get; set; } = 3000;

    [UserSetting("P6 分散分摊标记")] public bool p6Mark { get; set; } = false;

    [UserSetting("P7 死亡轮回116分摊")] public bool p7_116 { get; set; } = true;
    ...
    [ScriptMethod(name: "P4 记录", eventType: EventTypeEnum.CancelAction, eventCondition: ["ActionId:29750"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P1 BossId", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:28532"],
        userControl: false)]
    public void P1_BossId(Event @event, ScriptAccessory accessory)
    {
        if (ParseObjectId(@event["SourceId"], out var sid)) p1GrenoId = sid;
    }

    [ScriptMethod(name: "P1 阶段记录", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:25300"],
        userControl: false)]
    public void P1_阶段记录(Event @event, ScriptAccessory accessory)
    {
        if (parse == 0) parse = 1;
        parse = Math.Round(parse + 0.1, 1);
        if (ParseObjectId(@event["SourceId"], out var sid)) p1AdelId = sid;
    }
    ...
    public void P1_钢铁(Event @event, ScriptAccessory accessory)
    {
        ...
        if (ParseObjectId(@event["SourceId"], out var sid)) dp.Owner = sid;
        dp.DestoryAt = 5000;
    ...
    public void P1_苍穹炽焰(Event @event, ScriptAccessory accessory)
    {
        ...
        if (ParseObjectId(@event["TargetId"], out var tid)) dp.Owner = tid;
        dp.DestoryAt = 5000;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    }
    ...
    public void P1_直线多维空间斩(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Scale = new Vector2(8, 70);
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = p1GrenoId;
        if (ParseObjectId(@event["TargetId"], out var tid)) dp.TargetObject = tid;
        dp.DestoryAt = 6000;
    ...
    public void P1_次元裂缝危险区(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Color = accessory.Data.DefaultDangerColor;
        if (ParseObjectId(@event["SourceId"], out var id)) dp.Owner = id;
        dp.DestoryAt = 60000;
        dp.Name = $"P1 次元裂缝危险区{id:X}";
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    }

    [ScriptMethod(name: "P1 次元裂缝危险区移除", eventType: EventTypeEnum.RemoveCombatant, eventCondition: ["DataId:13071"],
        userControl: false)]
    public void P1_次元裂缝危险区移除(Event @event, ScriptAccessory accessory)
    {
        if (ParseObjectId(@event["SourceId"], out var id)) accessory.Method.RemoveDraw($"P1 次元裂缝危险区{id:X}");
    }

    [ScriptMethod(name: "P1 光芒剑阿代尔斐尔位置(ImGui)", eventType: EventTypeEnum.Targetable,
        eventCondition: ["Targetable:True"])]
    ...
    public void P1_光芒剑(Event @event, ScriptAccessory accessory)
    {
        ...
        if (float.TryParse(@event["SourceRotation"], out var r))
        {
            if (MathF.Abs(r + float.Pi / 4) < 0.1 || MathF.Abs(r - float.Pi * 0.75f) < 0.1)
            {
                ...
                dp.Position = new Vector3(111, 0, 111);
                accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
                dp.Name = "P1_光芒剑(89.00,89.00)";
                dp.Position = new Vector3(89, 0, 89);
                accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
            }
            ...
            if (MathF.Abs(r - float.Pi / 4) < 0.1 || MathF.Abs(r + float.Pi * 0.75f) < 0.1)
            {
                ...
                dp.Position = new Vector3(111, 0, 89);
                accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
                dp.Name = "P1_光芒剑(89.00,111.00)";
                dp.Position = new Vector3(89, 0, 111);
                accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
            }
        }

        dp.Name = "P1_光芒剑(78.00,100.00)";
        dp.Position = new Vector3(78, 0, 100);
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
        dp.Name = "P1_光芒剑(92.52,100.00)";
        dp.Position = new Vector3(92.52f, 0, 100);
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
        dp.Name = "P1_光芒剑(107.48,100.00)";
        dp.Position = new Vector3(107.48f, 0, 100);
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
        dp.Name = "P1_光芒剑(122.00,100.00)";
        dp.Position = new Vector3(122, 0, 100);
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
        dp.Name = "P1_光芒剑(100.00,78.00)";
        dp.Position = new Vector3(100, 0, 78);
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
        dp.Name = "P1_光芒剑(100,92.52.00)";
        dp.Position = new Vector3(100, 0, 92.52f);
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
        dp.Name = "P1_光芒剑(100.00,107.48)";
        dp.Position = new Vector3(100, 0, 107.48f);
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
        dp.Name = "P1_光芒剑(100.00,122.00)";
        dp.Position = new Vector3(100, 0, 122);
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    }

    [ScriptMethod(name: "P1 光球爆炸范围移除", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:25295"],
        userControl: false)]
    ...
    {
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Scale = new Vector2(1.5f, 16);
        dp.Color = accessory.Data.DefaultDangerColor.WithW(2);
        dp.Owner = accessory.Data.Me;
        if (ParseObjectId(@event["SourceId"], out var sid)) dp.TargetObject = sid;
    ...
    [ScriptMethod(name: "P2 1运记录", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:25555"],
        userControl: false)]
    public void P2_1运记录(Event @event, ScriptAccessory accessory)
    {
        parse = 2.1;
        firstTargetIcon = null;
        if (ParseObjectId(@event["SourceId"], out var id)) tordanId = id;
    }
    ...
    public void P2_1运冲锋位置记录(Event @event, ScriptAccessory accessory)
    {
        ...
        var dir = PositionTo8Dir(sourcePos, new Vector3(100, 0, 100));
        if (dir == 0 || dir == 4)
        {
            p2SafeDir[0] = false;
            p2SafeDir[4] = false;
    ...
    public void P2_1运穿天记录(Event @event, ScriptAccessory accessory)
    {
        ...
        if (ParseObjectId(@event["TargetId"], out var id)) p2BlueCircle.Add(id);
    }
    ...
    public void P2_1运穿天(Event @event, ScriptAccessory accessory)
    {
        ...
        if (ParseObjectId(@event["TargetId"], out var id)) dp.Owner = id;
        dp.Delay = 6000;
        dp.DestoryAt = 3000;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    }
    ...
    public void P2_一运穿天连线(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.ScaleMode |= ScaleMode.YByDistance;
        dp.DestoryAt = 9000;
        for (var i = 1; i < p2BlueCircle.Count; i++)
        {
    ...
    [ScriptMethod(name: "P2 二运记录", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:25569"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P2 二运龙眼背对", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["DirectorId:8003759A", "Id:00020001"])]
    public void P2_二运龙眼背对(Event @event, ScriptAccessory accessory)
    {
        if (parse != 2.2) return;
        var index = int.Parse(@event["Index"], NumberStyles.HexNumber);
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = "P2_二运龙眼背对";
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = accessory.Data.Me;
        dp.Delay = 4500;
        dp.DestoryAt = 5000;
        if (index == 0) dp.TargetPosition = new Vector3(100, 0, 65);
        if (index == 1) dp.TargetPosition = new Vector3(124.75f, 0, 75.25f);
        if (index == 2) dp.TargetPosition = new Vector3(135, 0, 100);
        if (index == 3) dp.TargetPosition = new Vector3(124.75f, 0, 124.75f);
        if (index == 4) dp.TargetPosition = new Vector3(100, 0, 135);
        if (index == 5) dp.TargetPosition = new Vector3(75.25f, 0, 124.75f);
        if (index == 6) dp.TargetPosition = new Vector3(65, 0, 100);
        if (index == 7) dp.TargetPosition = new Vector3(75.25f, 0, 75.25f);
    ...
    public void P2_二运骑神背对(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.Owner = accessory.Data.Me;
        if (ParseObjectId(@event["TargetId"], out var id)) dp.TargetObject = id;
        dp.DestoryAt = 5000;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.SightAvoid, dp);
    }

    [ScriptMethod(name: "P2 二运泽菲兰位置记录", eventType: EventTypeEnum.NpcYell, eventCondition: ["Id:2549"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P2 二运阿代尔菲尔位置", eventType: EventTypeEnum.SetObjPos, eventCondition: ["SourceDataId:12601"],
        userControl: false)]
    ...
    public void P2_二运光球爆炸范围(Event @event, ScriptAccessory accessory)
    {
        ...
        if (ParseObjectId(idStr, out var id)) dp.Owner = id;
        dp.DestoryAt = 2000;
    ...
    [ScriptMethod(name: "P2 光球爆炸范围移除", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:25295"],
        userControl: false)]
    ...
    public void P2_2运陨石记录(Event @event, ScriptAccessory accessory)
    {
        ...
        if (s1 != s2)
        {
            p2StoneMem = (s1, s2);
            //AB mt h2
            if (s1 == 0 && s2 == 3) p2StoneTeam = [0, 4, 5, 1, 3, 7, 6, 2];
            //AB d14
            if (s1 == 4 && s2 == 7) p2StoneTeam = [4, 0, 5, 1, 7, 3, 6, 2];
            //AC 双t
            if (s1 == 0 && s2 == 1) p2StoneTeam = [0, 4, 7, 3, 1, 5, 6, 2];
            //AC d12
            if (s1 == 4 && s2 == 5) p2StoneTeam = [4, 0, 7, 3, 5, 1, 6, 2];
            //AD mt H1
            if (s1 == 0 && s2 == 2) p2StoneTeam = [0, 4, 7, 3, 2, 6, 5, 1];
            //AD d13
            if (s1 == 4 && s2 == 6) p2StoneTeam = [4, 0, 7, 3, 6, 2, 5, 1];
            //BC h2 st
            if (s1 == 1 && s2 == 3) p2StoneTeam = [3, 7, 4, 0, 1, 5, 6, 2];
            //BC d24
            if (s1 == 5 && s2 == 7) p2StoneTeam = [7, 3, 4, 0, 5, 1, 6, 2];
            //BD h12
            if (s1 == 2 && s2 == 3) p2StoneTeam = [4, 0, 3, 7, 5, 1, 2, 6];
            //BD d34
            if (s1 == 6 && s2 == 7) p2StoneTeam = [4, 0, 7, 3, 5, 1, 6, 2];
            //CD st h1
            if (s1 == 1 && s2 == 2) p2StoneTeam = [2, 6, 7, 3, 1, 5, 4, 0];
            //CD d23
            if (s1 == 5 && s2 == 6) p2StoneTeam = [6, 2, 7, 3, 5, 1, 4, 0];
        }
    }
    ...
    public void P2_2运冰分摊位置(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(100).ContinueWith(t =>
        {
            ...
            dp.Scale = new Vector2(3f, 10);
            dp.ScaleMode |= ScaleMode.YByDistance;
            dp.Color = accessory.Data.DefaultSafeColor;
            dp.Owner = accessory.Data.Me;
            dp.TargetPosition = RotatePoint(new Vector3(100, 0, 88.5f), new Vector3(100, 0, 100), float.Pi / 2 * dir4);
            dp.DestoryAt = 7000;
    ...
    [ScriptMethod(name: "P2 二运第一轮塔记录", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:29564"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P2 二运第一轮塔位置(ImGui)", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:29563"])]
    public void P2_二运第一轮塔位置(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(100).ContinueWith(t =>
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                ...
                //右
                if (p2Tower[i * 3 + 2]) towerMem[i * 3 + 2] = MemIndex;
            }
            ...
            if (tIndex >= 0 && tIndex < 12) dp.Position = RotatePoint(npos, cpos, float.Pi / 6 * (tIndex - 1));
    ...
    [ScriptMethod(name: "P2 二运第二轮塔位置(ImGui)", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:28650"])]
    ...
    [ScriptMethod(name: "P2 二运结束记录", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:25533"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P3 记录", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:26376"],
        userControl: false)]
    public void P3_记录(Event @event, ScriptAccessory accessory)
    {
        parse = 3;
        if (ParseObjectId(@event["SourceId"], out var id)) p3BossId = id;
    }
    ...
    public void P3_四塔武神枪引导(Event @event, ScriptAccessory accessory)
    {
        ...
        if (ParseObjectId(str, out var id)) dp.Owner = id;
        dp.Scale = new Vector2(8, 62);
        dp.Delay = 5000;
        dp.DestoryAt = 2500;
        dp.TargetResolvePattern = PositionResolvePatternEnum.PlayerNearestOrder;
    ...
    public void P3_四塔记录(Event @event, ScriptAccessory accessory)
    {
        ...
        var sourcePos = JsonConvert.DeserializeObject<Vector3>(@event["SourcePosition"]);
        var dir = PositionTo8Dir(sourcePos, new Vector3(100, 0, 100)) / 2;
        p3Tower[dir] = num;
    }
    ...
    public void P3_四塔站位(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(100).ContinueWith(t =>
        {
            ...
            var dp = accessory.Data.GetDefaultDrawProperties();
            dp.Color = accessory.Data.DefaultSafeColor;
            dp.Position = RotatePoint(new Vector3(108, 0, 92), new Vector3(100, 0, 100), float.Pi / 2 * myTower);
            dp.Scale = new Vector2(5);
            dp.DestoryAt = 5000;
            accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Circle, dp);
        });
    ...
    [ScriptMethod(name: "P5 一运记录", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:27529"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P5 一运白龙位置连线(ImGui)", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:27531"])]
    public void P5_一运白龙位置连线(Event @event, ScriptAccessory accessory)
    {
        if (parse != 5.1) return;
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Color = accessory.Data.DefaultDangerColor;
        if (ParseObjectId(@event["SourceId"], out var id)) dp.TargetObject = id;
        dp.Owner = accessory.Data.Me;
        dp.Scale = new Vector2(5);
        dp.ScaleMode |= ScaleMode.YByDistance;
        dp.DestoryAt = 6000;
    ...
    [ScriptMethod(name: "P5 一运双龙俯冲处理位置记录", eventType: EventTypeEnum.SetObjPos, eventCondition: ["SourceDataId:12603"],
        userControl: false)]
    public void P5_一运双龙俯冲处理位置记录(Event @event, ScriptAccessory accessory)
    {
        if (parse != 5.1) return;
        var pos = JsonConvert.DeserializeObject<Vector3>(@event["SourcePosition"]);
        p5DivePos = new Vector3((pos.X - 100) / 9 * 19 + 100, pos.Y, (pos.Z - 100) / 9 * 19 + 100);
    }
    ...
    [ScriptMethod(name: "P5 一运格里诺位置记录", eventType: EventTypeEnum.SetObjPos, eventCondition: ["SourceDataId:12602"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P5 二运记录", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:27538"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P5 二运龙眼背对", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["DirectorId:8003759A", "Id:00020001"])]
    ...
    [ScriptMethod(name: "P5 二运骑神背对", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["DirectorId:8003759A", "Id:00020001"])]
    ...
    [ScriptMethod(name: "P5 二运死宣记录", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:2976"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P5 二运盖里克位置记录", eventType: EventTypeEnum.SetObjPos, eventCondition: ["SourceDataId:12637"],
        userControl: false)]
    ...
    public void P5_二运死宣六方站位(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(100).ContinueWith(t =>
        {
            ...
            var cpos = new Vector3(100, 0, 100);
            var npos = 19.5f * Vector3.Normalize(new Vector3(p5GreekPos.X - 100, p5GreekPos.Y, p5GreekPos.Z - 100)) +
                       cpos;
            if (posid == 4 || posid == 7)
                npos = 13 * Vector3.Normalize(new Vector3(p5GreekPos.X - 100, p5GreekPos.Y, p5GreekPos.Z - 100)) + cpos;
            var dp = accessory.Data.GetDefaultDrawProperties();
            dp.Color = accessory.Data.DefaultSafeColor;
            dp.Owner = accessory.Data.Me;
            dp.Scale = new Vector2(1.5f, 60);
            dp.DestoryAt = 7000;
            dp.ScaleMode |= ScaleMode.YByDistance;
    ...
    [ScriptMethod(name: "P5 二运索尼引导站位(ImGui)", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:27533"])]
    public void P5_二运索尼引导站位(Event @event, ScriptAccessory accessory)
    {
        ...
        var cpos = new Vector3(100, 0, 100);
        var npos = 10 * Vector3.Normalize(new Vector3(p5GreekPos.X - 100, p5GreekPos.Y, p5GreekPos.Z - 100)) + cpos;
    ...
    public void P5_二运索尼处理位置_横排法(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(100).ContinueWith(ca =>
        {
            ...
            var cpos = new Vector3(100, 0, 100);
            var npos = 4 * Vector3.Normalize(new Vector3(p5GreekPos.X - 100, p5GreekPos.Y, p5GreekPos.Z - 100)) + cpos;
            var npos2 = 20f * Vector3.Normalize(new Vector3(p5GreekPos.X - 100, p5GreekPos.Y, p5GreekPos.Z - 100)) +
                        cpos;
    ...
    [ScriptMethod(name: "P6 开场记录", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:26215"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P6 阶段累加", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:27969"],
        userControl: false)]
    public void P6_阶段累加(Event @event, ScriptAccessory accessory)
    {
        parse = Math.Round(parse + 0.1, 1);
        if (ParseObjectId(@event["SourceId"], out var id)) whiteDragonId = id;
    }

    [ScriptMethod(name: "P6 黑龙ID", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:27971"],
        userControl: false)]
    public void P6_黑龙ID(Event @event, ScriptAccessory accessory)
    {
        if (ParseObjectId(@event["SourceId"], out var id)) darkDragonId = id;
    }

    [ScriptMethod(name: "P6 白龙位置id记录", eventType: EventTypeEnum.SetObjPos, eventCondition: ["SourceDataId:12613"],
        userControl: false)]
    public void P6_白龙位置id记录(Event @event, ScriptAccessory accessory)
    {
        ...
        if (ParseObjectId(@event["SourceId"], out var sid)) whiteDragonId = sid;
    }
    ...
    [ScriptMethod(name: "P6 第一次冰火线站位(Imgui)", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:27960"])]
    public void P6_第一次冰火线站位(Event @event, ScriptAccessory accessory)
    {
        ...
        if (idIndex > 1) accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
    }
    ...
    [ScriptMethod(name: "P6 无尽轮回", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:27969"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P6 灭杀誓言分散", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:27960"],
        userControl: false)]
    public void P6_灭杀誓言分散(Event @event, ScriptAccessory accessory)
    {
        ...
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.DestoryAt = 7300;
        for (var i = 4; i < accessory.Data.PartyList.Count; i++)
        {
    ...
    [ScriptMethod(name: "P6 第一次冰火线安全点(ImGui)", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:26215"])]
    public void P6_第一次冰火线安全点(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Scale = new Vector2(0.2f);
        dp.Delay = 20000;
        dp.DestoryAt = 15000;
        dp.Name = "P6 第一次冰火线安全点";

        dp.Position = new Vector3(95.7f, 0, 119);
        accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Circle, dp);
        dp.Position = new Vector3(104.3f, 0, 119);
        accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Circle, dp);
        dp.Position = new Vector3(100, 0, 109.33f);
        accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Circle, dp);
    }
    ...
    public void P6_神圣之翼左近(Event @event, ScriptAccessory accessory)
    {
        ...
        dp2.Scale = new Vector2(22, 50);
        dp2.Owner = id;
        dp2.DestoryAt = 8000;
        dp2.Offset = new Vector3(-11, 0, 0);
        dp2.Name = "P6_神圣之翼左";
    ...
    public void P6_第二次冰火线ND站位(Event @event, ScriptAccessory accessory)
    {
        ...
        if (idIndex == 2) dp.TargetPosition = new Vector3(100, 0, 80.5f);
        if (idIndex == 3) dp.TargetPosition = new Vector3(100, 0, 119.7f);
        if (idIndex == 4) dp.TargetPosition = new Vector3(103.7f, 0, 89.2f);
        if (idIndex == 5) dp.TargetPosition = new Vector3(97, 0, 110.2f);
        if (idIndex == 6) dp.TargetPosition = new Vector3(107.2f, 0, 81.7f);
        if (idIndex == 7) dp.TargetPosition = new Vector3(92.5f, 0, 118);
    ...
    [ScriptMethod(name: "P6 暗buff记录", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:2758"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P6 光buff记录", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:2759"],
        userControl: false)]
    ...
    public void P6_邪念之炎(Event @event, ScriptAccessory accessory)
    {
        Task.Delay(18000).ContinueWith(t =>
        {
            ...
            for (var i = 0; i < p6lightDark.Count; i++)
            {
                ...
                if (p6lightDark[i] == 2)
                {
                    ...
                    dp.DestoryAt = 5000;
                    dp.Name = "P6 同归于尽之炎";
                    if (i == idIndex || (p6lightDark.IndexOf(2) == i && p6lightDark.IndexOf(0) == idIndex) ||
                        (p6lightDark.LastIndexOf(2) == i && p6lightDark.LastIndexOf(0) == idIndex))
                        dp.Color = accessory.Data.DefaultSafeColor;
                    else
                        dp.Color = accessory.Data.DefaultDangerColor;
                    accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    ...
    [ScriptMethod(name: "P6 邪念之炎/同归于尽之炎标记", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:27974"],
        userControl: false)]
    public void P6_邪念之炎标记(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(23000).ContinueWith(t => { accessory.Method.MarkClear(); });
    }
    ...
    [ScriptMethod(name: "P7 开场记录", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:29752"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P7 阶段累加地火", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:28059"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P7 阶段累加死亡轮回", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:28051"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P7 阶段累加陨石", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:28057"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P7 钢铁", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:2056", "StackCount:42"])]
    ...
    [ScriptMethod(name: "P7 月环", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:2056", "StackCount:43"])]
    ...
    public void P7_死亡轮回剑分摊处(Event @event, ScriptAccessory accessory)
    {
        Task.Delay(50).ContinueWith(t =>
        {
            ...
            if (isme)
            {
                ...
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.Scale = new Vector2(4);
                if (ParseObjectId(@event["SourceId"], out var sid2)) dp.Owner = sid2;
                dp.DestoryAt = 12000;
                accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
            }
        });
    ...
    [ScriptMethod(name: "P7 一号核爆位置收集", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:28058"],
        userControl: false)]
    ...
    [ScriptMethod(name: "P7 二号核爆位置收集", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:28114"],
        userControl: false)]
