...
[ScriptType(name: "M4s绘图", territorys: [1232], guid: "e7f7c69b-cc82-4b74-b1ea-2f3f0eecb2e2", version: "0.0.0.13",
    author: "Karlin")]
public class M4s绘图绘图
{
    ...
    private readonly bool[] isYelowBuff = [false, false, false, false, false, false, false, false];
    private readonly List<bool> long3999 = [false, false, false, false, false, false, false, false];
    private readonly bool[] long4000 = [false, false, false, false, false, false, false, false];


    private List<int> FloorFire = [0, 0, 0];
    private bool P1LightingDone;
    private List<int> TetherBoom = [0, 0, 0, 0, 0, 0, 0, 0];
    private List<uint> TetherLighting = [];
    private float fieldLaserDis;

    private int? firstTargetIcon;
    private int floorSpreadCount;
    private bool imP1lighting;
    private bool isEastYellowLaser;
    private bool isSouthYellowLaser;
    private List<bool> isYellowGun = [false, false, false, false];

    private int laserObjCount;

    private List<int> lightingCounts = [0, 0, 0, 0, 0, 0, 0, 0];
    private int parse;
    private bool southSafe1st;
    private bool southSafe2nd;
    private int spreadStackCount;
    private bool stackBuff;
    private string storeSkill = "";

    private DateTime timeLock;
    private int towerCount;
    private uint towerId;

    [UserSetting("奔雷炮站位方式")] public LaserPositionEnum LaserPosition { get; set; }

    [UserSetting("Debug模式")] public bool DebugMode { get; set; } = true;
    ...
    [ScriptMethod(name: "本体记录", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:38386"],
        userControl: false)]
    ...
    [ScriptMethod(name: "雷电buff收集", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:587"],
        userControl: false)]
    public void 雷电buff收集(Event @event, ScriptAccessory accessory)
    {
        if (!ParseObjectId(@event["TargetId"], out var tid)) return;
        if (tid == accessory.Data.Me) imP1lighting = true;
    }
    ...
    [ScriptMethod(name: "雷电buff分散站位", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:2970", "StackCount:regex:^(24[67])$"])]
    ...
    [ScriptMethod(name: "钢铁月环组合技-钢铁月环", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3836[89])$"])]
    public void 钢铁月环组合技钢铁月环(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Scale = isCircleFirst ? new Vector2(10) : new Vector2(30);
    ...
    [ScriptMethod(name: "钢铁月环组合技-处理位置", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3836[89])$"])]
    public void 钢铁月环组合技处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        //38368 月钢
        //15-3.5-3.5-3.5
        var pos = new Vector3[8, 4];

        if (@event["ActionId"] == "38368")
        {
            //ok
            pos[0, 0] = new Vector3(100.0f, 0, 089.3f);
            pos[0, 1] = new Vector3(100.0f, 0, 096.0f);
            pos[0, 2] = new Vector3(100.0f, 0, 087.8f);
            pos[0, 3] = new Vector3(100.0f, 0, 094.5f);
            pos[1, 0] = new Vector3(100.0f, 0, 110.7f);
            pos[1, 1] = new Vector3(100.0f, 0, 104.0f);
            pos[1, 2] = new Vector3(100.0f, 0, 112.2f);
            pos[1, 3] = new Vector3(100.0f, 0, 105.5f);

            pos[2, 0] = new Vector3(090.0f, 0, 110.0f);
            pos[2, 1] = new Vector3(093.3f, 0, 106.7f);
            pos[2, 2] = new Vector3(091.5f, 0, 108.5f);
            pos[2, 3] = new Vector3(094.0f, 0, 106.0f);
            pos[3, 0] = new Vector3(110.0f, 0, 110.0f);
            pos[3, 1] = new Vector3(106.7f, 0, 106.7f);
            pos[3, 2] = new Vector3(108.5f, 0, 108.5f);
            pos[3, 3] = new Vector3(106.0f, 0, 106.0f);

            pos[4, 0] = new Vector3(087.8f, 0, 100.0f);
            pos[4, 1] = new Vector3(094.5f, 0, 100.0f);
            pos[4, 2] = new Vector3(089.3f, 0, 100.0f);
            pos[4, 3] = new Vector3(096.0f, 0, 100.0f);
            pos[5, 0] = new Vector3(112.2f, 0, 100.0f);
            pos[5, 1] = new Vector3(105.5f, 0, 100.0f);
            pos[5, 2] = new Vector3(110.7f, 0, 100.0f);
            pos[5, 3] = new Vector3(104.0f, 0, 100.0f);

            pos[6, 0] = new Vector3(091.5f, 0, 091.5f);
            pos[6, 1] = new Vector3(094.0f, 0, 094.0f);
            pos[6, 2] = new Vector3(090.0f, 0, 090.0f);
            pos[6, 3] = new Vector3(093.3f, 0, 093.3f);
            pos[7, 0] = new Vector3(108.5f, 0, 091.5f);
            pos[7, 1] = new Vector3(106.0f, 0, 094.0f);
            pos[7, 2] = new Vector3(110.0f, 0, 090.0f);
            pos[7, 3] = new Vector3(106.7f, 0, 093.3f);
        }
        else
        {
            pos[0, 0] = new Vector3(100.0f, 0, 096.0f);
            pos[0, 1] = new Vector3(100.0f, 0, 089.3f);
            pos[0, 2] = new Vector3(100.0f, 0, 094.5f);
            pos[0, 3] = new Vector3(100.0f, 0, 087.8f);
            pos[1, 0] = new Vector3(100.0f, 0, 104.0f);
            pos[1, 1] = new Vector3(100.0f, 0, 110.7f);
            pos[1, 2] = new Vector3(100.0f, 0, 105.5f);
            pos[1, 3] = new Vector3(100.0f, 0, 112.2f);

            pos[2, 0] = new Vector3(093.3f, 0, 106.7f);
            pos[2, 1] = new Vector3(090.0f, 0, 110.0f);
            pos[2, 2] = new Vector3(094.0f, 0, 106.0f);
            pos[2, 3] = new Vector3(091.5f, 0, 108.5f);
            pos[3, 0] = new Vector3(106.7f, 0, 106.7f);
            pos[3, 1] = new Vector3(110.0f, 0, 110.0f);
            pos[3, 2] = new Vector3(106.0f, 0, 106.0f);
            pos[3, 3] = new Vector3(108.5f, 0, 108.5f);

            pos[4, 0] = new Vector3(094.5f, 0, 100.0f);
            pos[4, 1] = new Vector3(087.8f, 0, 100.0f);
            pos[4, 2] = new Vector3(096.0f, 0, 100.0f);
            pos[4, 3] = new Vector3(089.3f, 0, 100.0f);
            pos[5, 0] = new Vector3(105.5f, 0, 100.0f);
            pos[5, 1] = new Vector3(112.2f, 0, 100.0f);
            pos[5, 2] = new Vector3(104.0f, 0, 100.0f);
            pos[5, 3] = new Vector3(110.7f, 0, 100.0f);

            pos[6, 0] = new Vector3(094.0f, 0, 094.0f);
            pos[6, 1] = new Vector3(091.5f, 0, 091.5f);
            pos[6, 2] = new Vector3(093.3f, 0, 093.3f);
            pos[6, 3] = new Vector3(090.0f, 0, 090.0f);
            pos[7, 0] = new Vector3(106.0f, 0, 094.0f);
            pos[7, 1] = new Vector3(108.5f, 0, 091.5f);
            pos[7, 2] = new Vector3(106.7f, 0, 093.3f);
            pos[7, 3] = new Vector3(110.0f, 0, 090.0f);
        }
    ...
    [ScriptMethod(name: "雷球方形aoe", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3834[56])$"])]
    public void 雷球方形aoe(Event @event, ScriptAccessory accessory)
    {
        ...
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = "雷球方形aoe";
        dp.Scale = isSmall ? new Vector2(10) : new Vector2(30);
        dp.Owner = sid;
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.DestoryAt = 7000;
    ...
    [ScriptMethod(name: "分散分摊Buff收集", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:2970", "StackCount:regex:^(24[01])$"], userControl: false)]
    public void 分散分摊Buff收集(Event @event, ScriptAccessory accessory)
    {
        stackBuff = @event["StackCount"] == "240";
    }

    [ScriptMethod(name: "左右刀计数", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(3838[01])$"],
        userControl: false)]
    ...
    [ScriptMethod(name: "左右刀分散分摊", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3838[01])$"])]
        ...
        Task.Delay(50).ContinueWith(t =>
        {
            if (spreadStackCount == 1)
            {
                if (stackBuff)
                {
                    ...
                    for (var i = 0; i < 4; i++)
                    {
                        ...
                        dp.Delay = 3000;
                        dp.DestoryAt = 4000;
                        dp.Color = MyIndex == i || parterner[MyIndex] == i
            ...
            if (spreadStackCount == 2)
            {
                if (stackBuff)
                {
                    ...
                    for (var i = 0; i < 8; i++)
                    {
                        var isLarge = (long3999[i] && lightingCounts[i] == 2) ||
                                      (!long3999[i] && lightingCounts[i] == 3);
                        if (isLarge)
                        {
                    ...
                    dp.Delay = 0;
                    dp.DestoryAt = 7000;
                    dp.Color = LargeTn.Contains(MyIndex)
                        ? accessory.Data.DefaultSafeColor
                        : accessory.Data.DefaultDangerColor;
                    accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
                    ...
                    dp.Delay = 0;
                    dp.DestoryAt = 7000;
                    dp.Color = SmallTn.Contains(MyIndex)
                        ? accessory.Data.DefaultSafeColor
                        : accessory.Data.DefaultDangerColor;
                    accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
                    ...
                    dp.Delay = 0;
                    dp.DestoryAt = 7000;
                    dp.Color = LargeDps.Contains(MyIndex)
                        ? accessory.Data.DefaultSafeColor
                        : accessory.Data.DefaultDangerColor;
                    accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
                    ...
                    dp.Delay = 0;
                    dp.DestoryAt = 7000;
                    dp.Color = SmalleDps.Contains(MyIndex)
                        ? accessory.Data.DefaultSafeColor
                        : accessory.Data.DefaultDangerColor;
                    accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
    ...
    [ScriptMethod(name: "第二次左右刀分散分摊 分摊位置", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3838[01])$"])]
    public void 第二次左右刀分散分摊分摊位置(Event @event, ScriptAccessory accessory)
    {
        Task.Delay(50).ContinueWith(t =>
        {
            ...
            if (spreadStackCount == 2 && stackBuff)
            {
                ...
                if (LargeTn.Contains(MyIndex)) dealPos = new Vector3(isEast ? 92 : 108, 0, 100);
                if (SmallTn.Contains(MyIndex)) dealPos = new Vector3(isEast ? 99 : 101, 0, 92);
                if (LargeDps.Contains(MyIndex)) dealPos = new Vector3(isEast ? 99 : 101, 0, 100);
                if (SmalleDps.Contains(MyIndex)) dealPos = new Vector3(isEast ? 99 : 101, 0, 108);
    ...
    [ScriptMethod(name: "雷击计数器", eventType: EventTypeEnum.ActionEffect, eventCondition: ["ActionId:38790"],
        userControl: false)]
    ...
    [ScriptMethod(name: "Buff3999计数器", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:3999"],
        userControl: false)]
    ...
    public void 地板分散位置(Event @event, ScriptAccessory accessory)
    {
        lock (this)
        {
            ...
            var meIsSmall = long3999[myIndex] & (lightingCounts[myIndex] == 1) ||
                            !long3999[myIndex] & (lightingCounts[myIndex] == 2);

            StringBuilder sb = new();
            for (var i = 0; i < 8; i++) sb.Append(long3999[i] ? "1" : "0");
            if (DebugMode)
                accessory.Method.SendChat(
                    $"/e 二雷 第{floorSpreadCount}轮 北{dir4} Me[{myIndex}号|{(long3999[myIndex] ? "长" : "短")}|{lightingCounts[myIndex]}次|{(meIsSmall ? "小" : "大")}] Buff[{sb}]");


            var dealPos = new Vector3();
            if (meIsSmall)
                dealPos = myIndex > 3 ? new Vector3(116, 0, 108) : new Vector3(84, 0, 108);
            else
                dealPos = myIndex > 3 ? new Vector3(116, 0, 84) : new Vector3(84, 0, 84);

            if (long3999[myIndex] && floorSpreadCount == 1) dealPos = new Vector3(100, 0, 116);
            if (!long3999[myIndex] && floorSpreadCount != 1) dealPos = new Vector3(100, 0, 116);
    ...
    [ScriptMethod(name: "黄蓝buff收集器", eventType: EventTypeEnum.StatusAdd, eventCondition: ["StatusID:regex:^(400[01])$"],
        userControl: false)]
    ...
    [ScriptMethod(name: "黄激光收集", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(3835[89])$"],
        userControl: false)]
    ...
    [ScriptMethod(name: "左右拆地板", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["ActionId:regex:^(3835[45])$"])]
    public void 左右拆地板(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Position = new Vector3(isEast ? 105 : 95, 0, 80);
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.DestoryAt = 7000;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Rect, dp);
    }
    ...
    public void 黄蓝激光(Event @event, ScriptAccessory accessory)
    {
        ...
        dp.Scale = new Vector2(10, 20);
        dp.Owner = sid;
        dp.Color = isYellowLaser == isYelowBuff[myIndex]
            ? accessory.Data.DefaultDangerColor
            : accessory.Data.DefaultSafeColor;
        dp.DestoryAt = 6000;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Rect, dp);
    ...
    [ScriptMethod(name: "激光buff处理位置", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(400[23456])$"])]
    public void 激光buff处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var dealPos = isEastYellowLaser ? new Vector3(115, 0, 100) : new Vector3(85, 0, 100);
        if (statusID == "4006")
            dealPos += isYelowBuff[myIndex] != isSouthYellowLaser ? new Vector3(0, 0, 6) : new Vector3(0, 0, -6);
        if (statusID == "4002" || statusID == "4003")
            dealPos += isYelowBuff[myIndex] != isSouthYellowLaser ? new Vector3(0, 0, 4.5f) : new Vector3(0, 0, -4.5f);
        if (statusID == "4004" || statusID == "4005")
        {
            dealPos += isYelowBuff[myIndex] != isSouthYellowLaser ? new Vector3(0, 0, 2.1f) : new Vector3(0, 0, -2.1f);
            if (LaserPosition == LaserPositionEnum.Game8)
            {
                var dvx = isEastYellowLaser ? 1 : -1;
                dealPos += myIndex > 3 ? new Vector3(-2.9f * dvx, 0, 0) : new Vector3(2.9f * dvx, 0, 0);
            }
            ...
            if (LaserPosition == LaserPositionEnum.MMW)
            {
                ...
                dealPos += isSourth ? new Vector3(2.9f * dvx, 0, 0) : new Vector3(-2.9f * dvx, 0, 0);
            }
        }
    ...
    [ScriptMethod(name: "地火收集", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:regex:^(38389)$"],
        userControl: false)]
    ...
    public void 地火安全区位置(Event @event, ScriptAccessory accessory)
    {
        Task.Delay(100).ContinueWith(t =>
        {
            lock (FloorFire)
            {
                ...
                dp.Scale = new Vector2(2);
                dp.Owner = accessory.Data.Me;
                dp.TargetPosition = new Vector3(isMtg ? 80.5f : 119.5f, 0, 165);
                dp.ScaleMode |= ScaleMode.YByDistance;
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.DestoryAt = 8000;
                accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
                ...
                dp.Name = "地火安全区位置";
                dp.Scale = new Vector2(2);
                dp.Position = new Vector3(isMtg ? 80.5f : 119.5f, 0, 165);
                dp.TargetPosition = new Vector3(isMtg ? 85f : 115f, 0, 150 + myLine * 10);
                dp.ScaleMode |= ScaleMode.YByDistance;
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.DestoryAt = 8000;
                accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
    ...
    [ScriptMethod(name: "接线爆炸重置", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:38430"],
        userControl: false)]
    ...
    [ScriptMethod(name: "接线爆炸收集器", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["TargetIndex:1", "ActionId:regex:^(3843[12])$"], userControl: false)]
    ...
    [ScriptMethod(name: "接线爆炸换buff同伴连线", eventType: EventTypeEnum.ActionEffect,
        eventCondition: ["TargetIndex:1", "ActionId:regex:^(38432)$"])]
    public void 接线爆炸换buff同伴连线(Event @event, ScriptAccessory accessory)
    {
        Task.Delay(100).ContinueWith(t =>
        {
            ...
            if (sort.IndexOf(none1) > sort.IndexOf(none2)) (none2, none1) = (none1, none2);
    ...
    [ScriptMethod(name: "接线爆炸换buff同伴连线删除", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:regex:^(4008)$"], userControl: false)]
    ...
    [ScriptMethod(name: "延迟左右技能存储", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3840[2345])$"], userControl: false)]
    ...
    [ScriptMethod(name: "延迟左右技能范围", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3840[6789])$"])]
    public void 延迟左右技能范围(Event @event, ScriptAccessory accessory)
    {
        ...
        if (fire)
        {
            ...
            dp.Position = new Vector3(left ? 90 : 110, 0, 165);
            dp.Color = accessory.Data.DefaultDangerColor;
            dp.DestoryAt = 6000;
            accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Circle, dp);
        ...
        else
        {
            ...
            dp.Owner = accessory.Data.Me;
            dp.TargetPosition = new Vector3(left ? 90 : 110, 0, 165);
    ...
    [ScriptMethod(name: "分身左右刀", eventType: EventTypeEnum.PlayActionTimeline,
        eventCondition: ["Id:regex:^(456[6789])$"])]
    ...
    [ScriptMethod(name: "分身月环十字", eventType: EventTypeEnum.PlayActionTimeline,
        eventCondition: ["Id:regex:^(456[1234])$"])]
    ...
    [ScriptMethod(name: "分身月环十字收集", eventType: EventTypeEnum.PlayActionTimeline,
        eventCondition: ["Id:regex:^(456[1234])$"], userControl: false)]
    public void 分身月环十字收集(Event @event, ScriptAccessory accessory)
    {
        //4561 先直线
        //4563 先月环
        var pos = JsonConvert.DeserializeObject<Vector3>(@event["SourcePosition"]);
        var dir8 = PositionTo8Dir(pos, new Vector3(100, 0, 165));
    ...
    [ScriptMethod(name: "分身月环十字 分散分摊", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3844[34])$"])]
    public void 分身月环十字分散分摊(Event @event, ScriptAccessory accessory)
    {
        ...
        if (spread)
        {
            ...
            for (var i = 0; i < 4; i++)
            {
                ...
                dp.Color = myIndex == ii || myIndex == parterner[ii]
                    ? accessory.Data.DefaultSafeColor
                    : accessory.Data.DefaultDangerColor;
                dp.Delay = 7000;
                dp.DestoryAt = 4000;
    ...
    [ScriptMethod(name: "分身月环十字 分散分摊处理位置", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3844[34])$"])]
    public void 分身月环十字分散分摊处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        if (spread)
        {
            ...
            if (southSafe1st)
                ...
                r1 = myIndex > 3 ? r : -r;
            var pos1 = RotatePoint(new Vector3(100, 0, 152), new Vector3(100, 0, 165),
                rots1[myIndex] * float.Pi / 4 - r1);
            var pos2 = RotatePoint(new Vector3(100, 0, 152), new Vector3(100, 0, 165), rots2[myIndex] * float.Pi / 4);
        ...
        else
        {
            ...
            if (southSafe2nd)
                ...
                r1 = myIndex > 3 ? r : -r;
            var pos1 = RotatePoint(new Vector3(100, 0, 152), new Vector3(100, 0, 165), rots1[myIndex] * float.Pi / 4);
            var pos2 = RotatePoint(new Vector3(100, 0, 152), new Vector3(100, 0, 165),
                rots2[myIndex] * float.Pi / 4 - r1);
    ...
    [ScriptMethod(name: "剑舞范围", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(3839[345])$"])]
    public void 剑舞范围(Event @event, ScriptAccessory accessory)
    {
        ...
        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = "剑舞范围-1";
        dp.Scale = new Vector2(12, 60);
        dp.Position = new Vector3(100, 0, 165);
        dp.Color = accessory.Data.DefaultDangerColor;
        dp.DestoryAt = dur;
        accessory.Method.SendDraw(DrawModeEnum.Default, DrawTypeEnum.Straight, dp);
        ...
        dp.Name = "剑舞范围-4";
        dp.Scale = new Vector2(12, 60);
        dp.Position = new Vector3(100, 0, 165 + dz);
    ...
    //0 10000
    //18000 10000
    [ScriptMethod(name: "分身双人塔Id记录", eventType: EventTypeEnum.PlayActionTimeline, eventCondition: ["Id:4565"],
        userControl: false)]
    ...
    public void 分身双人塔处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        if (long4000[myIndex])
        {
            ...
            //%会有精度问题
            var dueFace = MathF.Abs(rot) < 0.1 || MathF.Abs(MathF.Abs(rot) - float.Pi / 2) < 0.1 ||
                          MathF.Abs(MathF.Abs(rot) - float.Pi) < 0.1;

            //accessory.Log.Debug($"rot{rot} {MathF.Abs(MathF.Abs(rot) % (float.Pi / 2))} dueFace {MathF.Abs(MathF.Abs(rot) % (float.Pi / 2))<0.1} {dueFace}");
            var dir4 = PositionRoundTo4Dir(sourcepos, new Vector3(100, 0, 165));
            var atEast = ((dir4 == 0 || dir4 == 2) && !dueFace) || ((dir4 == 1 || dir4 == 3) && dueFace);
            if (DebugMode)
                accessory.Method.SendChat(
                    $"/e 日出1 Me{myIndex} {(dueFace ? "正" : "斜")}-{(atEast ? "东西" : "南北")} [{rot:f2}|{sourcepos.X:F2},{sourcepos.Z:F2}]");
            Vector3 dealpos = default;
            if (myIndex < 4) dealpos = atEast ? new Vector3(85, 0, 165) : new Vector3(100, 0, 180);
            else dealpos = atEast ? new Vector3(115, 0, 165) : new Vector3(100, 0, 150);
        ...
        else
        {
            Task.Delay(15000).ContinueWith(t =>
            {
                ...
                if (DebugMode)
                    accessory.Method.SendChat(
                        $"/e 日出2 {(dueFace ? "正" : "斜")}-{(atEast ? "东西" : "南北")} [{rot:f2}|{sourcepos.X:F2},{sourcepos.Z:F2}]");
                Vector3 dealpos = default;
                if (myIndex < 4) dealpos = atEast ? new Vector3(85, 0, 165) : new Vector3(100, 0, 180);
                else dealpos = atEast ? new Vector3(115, 0, 165) : new Vector3(100, 0, 150);
    ...
    [ScriptMethod(name: "分身大炮处理位置", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:2970", "StackCount:regex:^(24[45])$"])]
    public void 分身大炮处理位置(Event @event, ScriptAccessory accessory)
    {
        ...
        var dir4 = PositionFloorTo4Dir(pos, new Vector3(100, 0, 165));
        var myIndex = accessory.Data.PartyList.IndexOf(accessory.Data.Me);
        lock (isYellowGun)
        {
            towerCount++;
            ...
            var atEast = ((towerdir4 == 0 || towerdir4 == 2) && !dueFace) ||
                         ((towerdir4 == 1 || towerdir4 == 3) && dueFace);
            if (DebugMode)
                accessory.Method.SendChat(
                    $"/e 日出3 Me{myIndex} {(dueFace ? "正" : "斜")}-{(atEast ? "东西" : "南北")} [{rot:f2}|{sourcepos.X:F2},{sourcepos.Z:F2}]");
            //{"X":110.58,"Y":-0.00,"Z":154.38}
            var myPosIndex = 0;
            if (isYelowBuff[myIndex])
                myPosIndex = myIndex > 3 ? isYellowGun.IndexOf(false) : isYellowGun.LastIndexOf(false);
            else myPosIndex = myIndex > 3 ? isYellowGun.IndexOf(true) : isYellowGun.LastIndexOf(true);

            Vector3 dealpos = default;
            if (myPosIndex == 0) dealpos = atEast ? new Vector3(107.6f, 0, 152.4f) : new Vector3(112.5f, 0, 157.5f);
            if (myPosIndex == 1) dealpos = atEast ? new Vector3(107.6f, 0, 177.6f) : new Vector3(112.5f, 0, 172.5f);
            if (myPosIndex == 2) dealpos = atEast ? new Vector3(092.4f, 0, 177.6f) : new Vector3(087.5f, 0, 172.5f);
            if (myPosIndex == 3) dealpos = atEast ? new Vector3(092.4f, 0, 152.4f) : new Vector3(087.5f, 0, 157.5f);
    ...
    [ScriptMethod(name: "分身大炮范围", eventType: EventTypeEnum.StatusAdd,
        eventCondition: ["StatusID:2970", "StackCount:regex:^(24[45])$"])]
