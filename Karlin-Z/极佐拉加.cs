...
[ScriptType(name: "极佐拉加绘图", territorys: [1201], guid: "d9c97e91-9b59-432d-a3a1-42a8586985b7e2a", version: "0.0.0.2",
    author: "Karlin")]
public class ZoraalaExDraw
{
    private readonly Vector3[] northPos =
    [
    ...
    private readonly float[] posXIndex = [66.41f, 69.94f, 73.48f, 77.02f, 122.92f, 126.52f, 130.06f, 133.59f];
    private readonly int[] sDic = [1, 2, 1, 2, 5, 6, 5, 6];

    private readonly Vector3[] southPos =
    [
    ...
    public void 无敌裂斩(Event @event, ScriptAccessory accessory)
    {
        ...
        {
            if (de.Y > 0)
                dp.Offset = new Vector3(0, 0, 30);
            else
                dp.Offset = new Vector3(30, 0, 0);
        }
        else
        {
            if (de.Y > 0)
    ...
    [ScriptMethod(name: "地板收集器", eventType: EventTypeEnum.EnvControl, eventCondition: ["Index:0000000B"],
        userControl: false)]
    public void 地板收集器(Event @event, ScriptAccessory accessory)
    {
        if (@event["Id"] == "02000100") dVector = new Vector3(21.21f, 0, 21.21f); //左上
        if (@event["Id"] == "00800040") dVector = new Vector3(-21.21f, 0, 21.21f); //右上
    ...
    [ScriptMethod(name: "连线冲收集器火内", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["Id:00020001", "Index:00000005"], userControl: false)]
    ...
    [ScriptMethod(name: "连线冲收集器火外", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["Id:00020001", "Index:00000008"], userControl: false)]
    public void 连线冲收集器火外(Event @event, ScriptAccessory accessory)
    {
        fireInside = false;
    }

    [ScriptMethod(name: "连线冲收集器swCross", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["Id:02000100", "Index:00000002"], userControl: false)]
    public void 连线冲收集器swCross(Event @event, ScriptAccessory accessory)
    {
        nDic = [1, 3, 0, 2, 6, 4, 7, 5];
    }

    [ScriptMethod(name: "连线冲收集器seCross", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["Id:02000100", "Index:00000003"], userControl: false)]
    ...
    public void 利刃冲(Event @event, ScriptAccessory accessory)
    {
        var pos = JsonConvert.DeserializeObject<Vector3>(@event["SourcePosition"]);
        var n = 0;
        for (var i = 0; i < posXIndex.Length; i++)
            if (MathF.Abs(pos.X - posXIndex[i]) < 1)
                n = i;
        if (pos.Z > 100) //南方普通
        {
            var dp = accessory.Data.GetDefaultDrawProperties();
        ...
        {
            var fire = false;
            if ((fireInside && (n == 2 || n == 3 || n == 4 || n == 5)) ||
                (!fireInside && (n == 0 || n == 1 || n == 6 || n == 7)))
                fire = true;
            var dp = accessory.Data.GetDefaultDrawProperties();
            dp.Name = fire ? "北方火冲" : "北方风冲";
            dp.Scale = fire ? new Vector2(15, 20) : new Vector2(5, 20);
