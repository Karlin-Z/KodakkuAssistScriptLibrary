...
[ScriptType(name: "M1s绘图", territorys: [1226], guid: "8010d865-7d6d-4c23-92e0-f4b0120e18ac", version: "0.0.0.9",
    author: "Karlin")]
public class M1sDraw
{
    ...
    private List<int> FloorBrokeList = new();
    private List<string> P3JumpSkill = new();
    private List<uint> P3TetherTarget = new();
    private uint copyCatTarget;

    private int? firstTargetIcon;
    private uint parse;

    [UserSetting("地板修复击退,Mt组安全半场")] public KnockBackMtPosition MtSafeFloor { get; set; }

    [UserSetting("分身跳跃预测颜色")] public ScriptColor jumpColor { get; set; } = new() { V4 = new Vector4(1, 1, 0, 1) };
    ...
    public void Init(ScriptAccessory accessory)
    {
        ...
        parse = 1;
        P3TetherTarget = new List<uint>();
        P3JumpSkill = new List<string>();
    }
    ...
    [ScriptMethod(name: "分身左右刀", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(37989|3799[023])$"])]
    ...
    [ScriptMethod(name: "跳跃左右刀", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^3796[5678]$"])]
    public void 跳跃左右刀(Event @event, ScriptAccessory accessory)
    {
        ...
        Vector3 dv;
        if (leftJump) dv = new Vector3(-10, 0, 0);
        else dv = new Vector3(10, 0, 0);

        var dp = accessory.Data.GetDefaultDrawProperties();
        dp.Name = $"跳跃左右刀-{(leftJump ? "左" : "右")}跳{(leftFast ? "左" : "右")}刀快";
        dp.Position = pos + dv;
        dp.Scale = new Vector2(60, 30);
    ...
    [ScriptMethod(name: "跳跃扇形引导", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(38959|37975)$"])]
    public void 跳跃扇形引导(Event @event, ScriptAccessory accessory)
    {
        //38959右扇
        //37975左扇
        var pos = JsonConvert.DeserializeObject<Vector3>(@event["SourcePosition"]);
        var dv = @event["ActionId"] == "37975" ? new Vector3(-10, 0, 0) : new Vector3(10, 0, 0);
        if (Math.Abs(pos.X - 110) < 1)
            dv = @event["ActionId"] == "37975" ? new Vector3(0, 0, 10) : new Vector3(0, 0, -10);
    ...
    [ScriptMethod(name: "P3跳跃技能收集", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(38959|37975|3796[5678])$"])]
    ...
    public void P3分身连线技能(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(100).ContinueWith(t =>
        {
            ...
            if (skillId == "38959" || skillId == "37975")
            {
                ...
                if (isNouthCopy)
                    dv = leftJump ? new Vector3(10, 0, 0) : new Vector3(-10, 0, 0);
                else
                    dv = leftJump ? new Vector3(-10, 0, 0) : new Vector3(10, 0, 0);
    ...
    public void P3拉怪辅助(Event @event, ScriptAccessory accessory)
    {
        ...
        Task.Delay(100).ContinueWith(t =>
        {
            ...
            if (P3TetherTarget.Count == 2)
            {
                ...
                {
                    if (isNouthCopy1)
                        epos = leftJump2 ? new Vector3(110, 0, 100) : new Vector3(90, 0, 100);
                    else
                        epos = leftJump2 ? new Vector3(90, 0, 100) : new Vector3(110, 0, 100);
                }


                var dp = accessory.Data.GetDefaultDrawProperties();
                dp.Name = "P3拉怪辅助起始";
                dp.Scale = new Vector2(2, 10);
            ...
            if (P3TetherTarget.Count == 3)
            {
                ...
                if (isNouthCopy1)
                    dv1 = leftJump1 ? new Vector3(10, 0, 0) : new Vector3(-10, 0, 0);
                else
                    dv1 = leftJump1 ? new Vector3(-10, 0, 0) : new Vector3(10, 0, 0);
                var dp = accessory.Data.GetDefaultDrawProperties();
                dp.Name = "P3拉怪辅助1";
                ...
                if (isNouthCopy1)
                    dv2 = leftJump2 ? new Vector3(-10, 0, 0) : new Vector3(10, 0, 0);
                else
                    dv2 = leftJump2 ? new Vector3(10, 0, 0) : new Vector3(-10, 0, 0);
                dp = accessory.Data.GetDefaultDrawProperties();
                dp.Name = "P3拉怪辅助2";
    ...
    [ScriptMethod(name: "双人分摊", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(37982|38016)$"])]
    ...
    [ScriptMethod(name: "四人分摊", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(37984|38018)$"])]
    ...
    {
        accessory.Method.TextInfo("职能分散", 5500);
    }

    [ScriptMethod(name: "地板破坏安全区重置", eventType: EventTypeEnum.StartCasting, eventCondition: ["ActionId:37953"],
        userControl: false)]
    public void 地板破坏安全区重置(Event @event, ScriptAccessory accessory)
    {
        FloorBrokeList = new List<int>();
    }

    [ScriptMethod(name: "地板破坏安全区", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(39276|37955)$"])]
    public void 地板破坏安全区(Event @event, ScriptAccessory accessory)
    {
        ...
        lock (FloorBrokeList)
        {
            ...
            if (FloorBrokeList.Count == 5)
            {
                ...
                {
                    var startPos = Math.Abs(FloorBrokeList[3] - safePosIndex1) % 4 == 1
                        ? IndexToFloor(FloorBrokeList[3])
                        : IndexToFloor(FloorBrokeList[2]);
                    var dp = accessory.Data.GetDefaultDrawProperties();
                    dp.Name = "地板破坏安全区";
                ...
                if (safePosBrokeIndex == 2)
                {
                    if (Math.Abs(FloorBrokeList[0] - safePosIndex1) % 4 == 1)
                    {
                        ...
                        dp.Position = pos2;
                        dp.TargetPosition =
                            new Vector3((endPos.X - 100) * 0.6f + 100, 0, (endPos.Z - 100) * 0.6f + 100);
                        dp.ScaleMode |= ScaleMode.YByDistance;
                        dp.Color = accessory.Data.DefaultSafeColor;
                        dp.DestoryAt = during;
                        accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Displacement, dp);
    ...
    [ScriptMethod(name: "分身砸地十字", eventType: EventTypeEnum.StartCasting,
        eventCondition: ["ActionId:regex:^(37960|37958)$"])]
    ...
    [ScriptMethod(name: "地板修复安全区", eventType: EventTypeEnum.EnvControl,
        eventCondition: ["Id:00080004", "Index:regex:^(0000000[1247])$"])]
    public void 地板修复安全区(Event @event, ScriptAccessory accessory)
    {
        ...
        if (MtSafeFloor == KnockBackMtPosition.SouthHalf || MtSafeFloor == KnockBackMtPosition.NorthHalf)
        {
            ...
            if (@event["Index"] == "00000001")
            {
                var dp = accessory.Data.GetDefaultDrawProperties();
                dp.Name = "地板修复安全区";
                dp.Scale = new Vector2(20f, 10);
                dp.Position = isNorthGroup ? new Vector3(90, 0, 85) : new Vector3(110, 0, 115);
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.DestoryAt = 9000;
            ...
            if (@event["Index"] == "00000002")
            {
                ...
                dp.Name = "地板修复安全区";
                dp.Scale = new Vector2(20f, 10);
                dp.Position = isNorthGroup ? new Vector3(110, 0, 85) : new Vector3(90, 0, 115);
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.DestoryAt = 9000;
                accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Straight, dp);
            }
            ...
            if (@event["Index"] == "00000004")
            {
                ...
                dp.Position = isNorthGroup ? new Vector3(85, 0, 90) : new Vector3(115, 0, 110);
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.DestoryAt = 9000;
                accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Straight, dp);
            }
            ...
            if (@event["Index"] == "00000007")
            {
                ...
                dp.Name = "地板修复安全区";
                dp.Scale = new Vector2(10, 20);
                dp.Position = isNorthGroup ? new Vector3(115, 0, 90) : new Vector3(85, 0, 110);
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.DestoryAt = 9000;
                accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Straight, dp);
            }
        ...
        else
        {
            ...
            if (@event["Index"] == "00000001")
            {
                ...
                dp.Name = "地板修复安全区";
                dp.Scale = new Vector2(20f, 10);
                dp.Position = isEastGroup ? new Vector3(110, 0, 115) : new Vector3(90, 0, 85);
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.DestoryAt = 9000;
                accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Straight, dp);
            }
            ...
            if (@event["Index"] == "00000002")
            {
                ...
                dp.Name = "地板修复安全区";
                dp.Scale = new Vector2(20f, 10);
                dp.Position = isEastGroup ? new Vector3(110, 0, 85) : new Vector3(90, 0, 115);
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.DestoryAt = 9000;
                accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Straight, dp);
            }
            ...
            if (@event["Index"] == "00000004")
            {
                ...
                dp.Name = "地板修复安全区";
                dp.Scale = new Vector2(10, 20);
                dp.Position = isEastGroup ? new Vector3(115, 0, 110) : new Vector3(85, 0, 90);
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.DestoryAt = 9000;
                accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Straight, dp);
            }
            ...
            if (@event["Index"] == "00000007")
            {
                ...
                dp.Name = "地板修复安全区";
                dp.Scale = new Vector2(10, 20);
                dp.Position = isEastGroup ? new Vector3(115, 0, 90) : new Vector3(85, 0, 110);
                dp.Color = accessory.Data.DefaultSafeColor;
                dp.DestoryAt = 9000;
                accessory.Method.SendDraw(DrawModeEnum.Imgui, DrawTypeEnum.Straight, dp);
            }
